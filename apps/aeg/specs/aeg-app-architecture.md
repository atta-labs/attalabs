...
The project registry is read from disk by `apps/aeg/web/studio/src/lib/aeg-fs/read-root.ts`. `findAegRoot()` walks up from `process.cwd()` looking for `packages/governance/projects.md` (the project registry — relocated from `aeg-root/projects.md`, `aeg-forge-state-v1` task 2) to confirm it has found the repo root, then returns that directory's `aeg-root/`; `readRegistry()` reads the registry itself from the same relocated path.

> **§0 refinement — both active and archived iterations read from a GitHub Milestone, not a topology file (`aeg-forge-state-v1` task 5, #429; #515).** Per D-110, an iteration's Goal/Lifecycle live on a GitHub Milestone titled exactly the iteration slug (open = active, closed = archived); `read-root.ts` derives it via `@atta/aeg-forge-state`'s `deriveIterationFromForge`, enumerating active slugs via `listActiveIterationSlugs` and archived slugs via `listArchivedIterationSlugs` (new, #515 — replaces the prior `aeg-root/iterations/completed/*.md` file read for archived iterations, since a closed Milestone's `iteration:<slug>`-labeled Issues, queried `--state all`, fully resolve every task). For an ACTIVE iteration, `dependsOn`/`conflictsWith` are additionally merged in from a legacy topology table when one still exists on disk (`mergeTaskEdgesFromFile`) — a best-effort enrichment for any pre-cutover file, not a required data source; no live iteration carries such a file after #512/#517, so this path is dormant in practice. A small, closed, non-growing set of pre-Milestone-era legacy iterations (no Milestone exists for them at all) is the one remaining exception read from `aeg-root/iterations/completed/*.md` directly — permanently, since deleting one requires a real Milestone to exist first, which is a Principal decision this PR does not make.
...
**Todo-badge dispatch-readiness overlay (#372 bundled finding).** For `todo`-status tasks only, `apps/aeg/web/studio/src/lib/forge/dispatch-readiness.ts` additionally computes `@atta/aeg-core`'s `checkDispatchReadiness` — the same function `bin/verify-dispatch.ts` calls — so the board's `Todo` badge can distinguish genuinely dispatchable tasks from ones a real gate still blocks (`Ready` vs. `Blocked · needs #N`, rendered by `todoDispatchVisual` in `_lib/status-display.ts`). This is display-only: it never touches `DerivedStatus`/`deriveIteration` above. As of **D-120** (2026-07-13), the gate no longer blocks on a predecessor row's provenance state (D-052 item 1, superseded — D-077's automated post-merge provenance posting made that signal moot); the badge now reflects only the still-live predicates (Issue-existence, D-078 rationale, `Depends-on`/`Conflicts-with`, prior-iteration-archival).
