...
> **§0 refinement — active iterations read from a GitHub Milestone, not the topology file (`aeg-forge-state-v1` task 5, #429).** Per D-110, an active iteration's Goal/Lifecycle now live on a GitHub Milestone titled exactly the iteration slug; `read-root.ts` derives it via `@atta/aeg-forge-state`'s `deriveIterationFromForge` (the same adapter task 3a/3b already cut the CLI gates over to), enumerating the set of active slugs by listing open Milestones (`listActiveIterationSlugs`). `dependsOn`/`conflictsWith` are still merged in from the topology table (`mergeTaskEdgesFromFile`) — the same deliberate, temporary narrowing `verify-coherence.ts`'s `deriveOrFallback` already applies, for the same live reason: an Issue's own "Dependency rationale" section can go stale when a later Planner amendment updates the dependency set in prose without editing the original literal `Depends-on:` line. The topology file is read directly only as a fallback when the forge itself is unreachable. **Archived** iterations (`aeg-root/iterations/completed/`) are unaffected — read from disk directly, permanently, per task 7 (#431)'s own scope: completed topology files are the historical record and are never migrated.
