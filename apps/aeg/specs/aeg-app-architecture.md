...
1. **The repo's AEG artifacts** — iteration topology (forge-native: a Milestone + `iteration:<slug>`-labeled Issues carrying the task↔Issue map, `depends-on` / `conflicts-with` edges, grouping; a small closed legacy set of pre-Milestone iterations still reads from `aeg-root/iterations/completed/*.md`, forge-sole-state task 1) and the backlogs (`specs/*-backlog.md`: the plan).
...
> **§0 refinement — both active and archived iterations read from a GitHub Milestone, not a topology file (`aeg-forge-state-v1` task 5, #429; forge-sole-state task 1).** Per D-110, an iteration's Goal/Lifecycle live on a GitHub Milestone titled exactly the iteration slug (open = active, closed = archived); `read-root.ts` derives it via `@atta/aeg-forge-state`'s `deriveIterationFromForge`, enumerating active slugs via `listActiveIterationSlugs` and archived slugs via `listArchivedIterationSlugs` (new, forge-sole-state task 1 — replaces the prior `aeg-root/iterations/completed/*.md` file read for archived iterations, since a closed Milestone's `iteration:<slug>`-labeled Issues, queried `--state all`, fully resolve every task). For an ACTIVE iteration, `dependsOn`/`conflictsWith` are additionally merged in from a legacy topology table when one still exists on disk (`mergeTaskEdgesFromFile`) — a best-effort enrichment for any pre-cutover file, not a required data source; no live iteration carries such a file after forge-sole-state task 1, so this path is dormant in practice. A small, closed, non-growing set of pre-Milestone-era legacy iterations (no Milestone exists for them at all) is the one remaining exception read from `aeg-root/iterations/completed/*.md` directly — permanently, since backfilling one requires a real Milestone to exist first; see `iterations/README.md` §11.
