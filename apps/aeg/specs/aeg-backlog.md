...
- **Deploy the enforcement layer from a self-contained source — AEG currently violates its own "depends on nothing but git + a forge" principle.** `state-machine.md` and the `aeg` skill both state AEG is agent-agnostic and tool-agnostic; that only holds if AEG's own logic doesn't live inside a Claude-Code-specific or GitHub-Actions-specific location. Audited 2026-07-01, in this repo:
  - **Genuinely self-contained today** (runnable with only `git` + `bun`, no specific CI platform or agent harness required): `aeg-root/` (the model), `aeg-project/` (state), `packages/aeg-core/` (the pure logic library — the destination of the `aeg-consolidation` refactor), and the `scripts/verify-*.ts` CLI oracles once that refactor lands.
  - **Not self-contained — hand-authored directly at host-mandated paths, with no canonical source and no generator anywhere in the repo:** `.github/workflows/*.yml` (GitHub Actions), `.claude/hooks/*.sh` + `.claude/settings.json` wiring (Claude-Code-specific — a different agent harness needs its own differently-shaped equivalent), `.husky/*` (git/Husky-specific).
  - **Even the one place docs *claim* a canonical-source-then-generated pattern already exists — skills — is unbuilt for AEG's own skills.** `aeg-root/skills/{aeg,aeg-roles,brief-authoring}/SKILL.md` are documented as canonical with `.claude/skills/` as "a generated view produced by `aeg generate-skills`" (D-039), but no `.claude/skills/aeg/` (or `aeg-roles`/`brief-authoring`) exists in this repo, and no `aeg generate-skills` command exists anywhere — confirmed by direct inspection, not assumed from the docs.
  - **The fix:** `aeg.sh` (or a sibling `aeg deploy` subcommand) generates the host-platform adapters — CI workflow YAML, git/Husky hooks, Claude-Code hook wiring, the `.claude/skills/` view — from the canonical `aeg-root/` + `packages/aeg-core/` source, the same way a build step produces a platform-specific artifact from portable source. This closes the gap for every repo that adopts AEG going forward, *and* for this repo's own current drift (its workflows/hooks were hand-authored directly, never deployed). Depends on `aeg-consolidation` landing first (the check logic must be pure and homed in `aeg-core` before there's a clean canonical source to generate CI/hook glue *from*).
