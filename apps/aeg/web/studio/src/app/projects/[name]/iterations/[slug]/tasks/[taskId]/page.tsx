...
export async function generateMetadata({ params }: { params: Promise<Params> }): Promise<Metadata> {
  ...
  return { title: `${taskId} · ${slug} · ${name} · AEG Studio` }
}
      ...
      {items.map((id) => {
        ...
        return (
          <NextLink
            key={id}
            variant='unstyled'
            href={`/projects/${projectName}/iterations/${iterationSlug}/tasks/${id}`}
            className={
              blocking
...
function BriefBody({
  ...
  return (
    <p className='font-sans text-sm text-muted-foreground/70'>
      ...
      AEG convention <span className='font-mono'>task/&lt;iteration&gt;/&lt;id&gt;</span>.
    </p>
  )
}
