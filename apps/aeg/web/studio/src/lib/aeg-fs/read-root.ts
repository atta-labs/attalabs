 ...
 * `packages/governance/`.
 *
 * Active vs. archived (`aeg-forge-state-v1` task 5, #429; forge-sole-state
 * task 1, per D-110): both derive purely from the forge — a GitHub Milestone
 ...
 * live iteration carries such a file after forge-sole-state task 1, so this
 * path is dormant in practice and only degrades gracefully if one ever
 * reappears. Archived iterations never had a file merge and never will —
 * their dependency edges resolve entirely from each closed Issue's own body.
 *
 ...
 * on-disk enumeration source left to fall back to (forge-sole-state task 1).
 */
