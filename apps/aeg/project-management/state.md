...
**Purpose:** what is true right now for AEG **the product** (the UI + `aeg.sh`). For ecosystem-wide state see root `project-management/state.md`. For the AEG **model**, see root `project-management/` (constitution, manual-flow, iterations, roles).
...
- `apps/aeg/project-management/{state,now}.md` — this state layer (per D-037)
