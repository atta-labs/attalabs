...
**Product context:** `apps/herald-ai/aeg-project/state.md`
...
Also patch the Herald section in `aeg-project/state.md`. Find the line:
```
### Herald — *standalone AttaLabs product; not part of Atta*
```
And replace the entire Herald section header line with:
```
### Herald — *standalone AttaLabs product; Phase 1 in progress — see `apps/herald-ai/aeg-project/state.md`*
```
Then add these two lines immediately after the new header, before the existing body text:
```
**Full state:** `apps/herald-ai/aeg-project/state.md` — read that file for Herald detail.
...
- [ ] `apps/herald-ai/aeg-project/now.md` updated: Phase 1 tasks marked complete, next 3 things set to Phase 2 tasks
