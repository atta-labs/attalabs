...
- **Engine migration + multi-vendor BYOK and model choice — move Herald's AI call onto `@atta/engine` + `@atta/adapter-langgraph`, and make Herald multi-LLM like Vāda.** Two coupled halves of one piece of work:
  ...
  **(a) Engine migration.** Herald has exactly one AI call (the skeptical-auditor match), today a direct Vercel-AI-SDK `generateText()` with a hardcoded vendor, the system prompt as a TS string (`lib/prompts.ts`), and a manual JSON parse/retry loop. Replace that with a Flow YAML (`apps/herald-ai/yamls/herald-auditor.yaml`, `solo` shape, `output_format: structured`) loaded via `loadFlow` + `compileFlow` and run through the LangGraph adapter — the same execution model every other product uses. `extractSignals()`, the SHA-256 cache, and `buildPartialReport()` fallback stay as-is. Full context, engine-investigation findings, target architecture, locked decisions (D-031–D-036), and open questions are captured in the handoff doc `apps/herald-ai/docs/engine-migration-context.md` (to be committed). Three known engine smells to handle/verify: structured-output adapter path for solo (verify before shipping), `compileFlow` `question` param passed empty with inputs via `customVars`, catalog-loader is Vāda-specific but not blocking. **(In progress as iteration `herald-onto-engine` — task 1 / PR #104 migrates the `/api/match` auditor; the auditor prompt now lives in `herald-auditor.yaml`. `output_format: structured` was found NOT needed — the engine returns text and Herald parses JSON-by-prompt-instruction like Vāda. See that iteration for live status.)**
...
- **Endpoint unification.** Fold `/api/match` (profile audit) and `/api/recruiter/batch` (bulk) into one `/api/audit` cell runner; `runSingleMatch` is the reusable per-pair cell. `BulkAudit` currently still calls `/api/recruiter/batch`. Prerequisite for the matrix work below. (The engine migration is the natural trigger to do this — see above. **This is `herald-onto-engine` task 2 — and it is where `SKEPTICAL_AUDITOR_PROMPT` finally gets deleted from `lib/prompts.ts`: task 1 had to keep the constant because `/api/recruiter/batch` still imports it, and task 1's brief walled batch off. When task 2 migrates batch onto the engine, the constant's last importer is gone and it is removed.**)
