...
**Last updated:** 2026-09-01 — § 4 corrected: the build-time UI generator no longer resolves the library live through the central-CMS resolver; it reads a repo-committed pin, and the app-chrome `LibraryProvider` feed is now the only path that still calls that resolver live. Prior revision 2026-08-18 (§ 4 stated the library-resolution invariant in prose, including what does and does not enforce it). Earlier substantive revision 2026-06-25 (post `herald-agents-v2` task 8 — owner routes under `/[username]`, topbar icon buttons; reconciled with the central-CMS theme/library resolution).
...
There is **one shared topbar** everywhere except the public profile's own shell, plus two icon-button slots:
...
- `extraActions` is the right-cluster slot in the shared `TopBar` (rendered immediately before `accountMenu`). It is a backwards-compatible slot — Vāda already uses it; Herald reuses it rather than introducing a new prop on `@atta/ui/topbar`.
...
- **Responsive collapse.** Below `md` the topbar shrinks to logo · `ColorSchemeToggle` · hamburger. Nav links, `extraActions` (Herald: Settings gear · Palette/Theme), and `accountMenu` (Sign out / Sign in) all move into the hamburger sheet, so the bar reads as a clean two-button row at narrow widths. The shared `@atta/ui/topbar` orchestrates this — `extraActions` and `accountMenu` are no longer mirrored in the mobile actions row; the sheet hosts them instead. Vāda's existing `extraActions` Settings button inherits the same placement (previously inaccessible on mobile; now lives in the sheet beneath the nav links).

### Public profile topbar (`/[username]`)

The public profile uses a **two-bar** structure, identical on desktop and mobile:

1. **Row 1** is the shared `TopBar` — logo left, no centered nav links (Bulk Audit / UI / Settings are removed from the profile topbar), theme + auth right. When `isOwner`, `extraActions` carries a Theme (Palette icon) button → `/{username}/ui` (the appearance editor) and a Settings (gear icon) button → `/{username}/settings` (this reverses the earlier "single Settings entry point" design, which is superseded).
...
**This section is the invariant's home, and the rule below is not renegotiable inside an ordinary task.** A change that would let app chrome render the visitor's library, or let the public profile render the build-time one, is a deliberate architectural reversal — it is decided first and edited second, never the other way round. Nothing fails the build when the two paths cross: no lint rule and no test covers library resolution. Three things hold the rule instead. The `(owner)`/`(profile)` route-group split described below makes the *accidental* crossing structurally impossible, since neither group can inherit the other's provider — but it does nothing about a deliberate edit to either layout. The `.vinaya/doc-owners` binding on `apps/herald-ai/web/src/app/[username]/**` points at this file, so a change to these routes normally drags this file into the same diff — but read that for what it is: the gate is satisfied by the *file* appearing, not this section, and it has two escapes, a `Doc-neutral:` declaration (honoured only when the matched diff is verified comment- or whitespace-only) and a `vinaya/waiver:docs` label a principal applies. It forces the question; it does not answer it. And a reader then has to agree with the answer. That exposure, not any mechanism, is what stands behind the rule — which is why the constraint is written out here in full.

- **App chrome** — main `HeraldTopBar`, Bulk Audit, onboarding, the owner appearance editor at `/[username]/ui`, and the owner Settings at `/[username]/settings` — MUST render the **build-time pinned library** (`@atta/ui/components`, aliased to `packages/ui/generated/herald/components.ts`, generated from `packages/ui/scripts/ui-library-pins.ts`'s `herald` entry — no longer from `heraldConfig.userInterface.library.id`, see below). This is the app's fixed design system. It is **not** user-configurable.
...
Concretely (inheriting the central-CMS resolution):

- **Where the alias's id comes from — no longer the central CMS resolver.** Theme metadata still lives centrally in the `attalabs` (`l5n0n8nn`) Sanity dataset, and `userInterface.theme` still resolves through the `cms` package's resolver (`getProductUiConfig` / `getProductConfig`) exactly as before. `userInterface.library` does not: the build-time UI generator (`scripts/generate-ui.ts`) reads a repo-committed pin (`packages/ui/scripts/ui-library-pins.ts`) instead, so `@atta/ui/components` → `packages/ui/generated/herald/components.ts` is fixed at whatever that file names for `herald` — it changes only when the pin file is edited and the app is rebuilt, never from a live Sanity read. This closed a real build non-determinism (two builds of the same commit could generate different output depending on when the CMS fetch happened to run), but it also means the alias no longer tracks `getProductConfig('herald').userInterface.library.id` at runtime — the two can now disagree.
- **The app-chrome `LibraryProvider` feed still calls that live resolver — a second, independently-live path.** `(app)/layout.tsx` and `(owner)/layout.tsx` (below) still call `getProductConfig('herald').userInterface.library.id` on every request and feed the result into `LibraryProvider`. Before the pin existed, this and the build-time alias were two reads of the same live value and could only diverge by request-timing accident. Now one side is frozen (the pin) and the other is still live, so they are two genuinely different mechanisms that happen to currently agree only because no app-chrome component actually calls `useComponents()` — every app-chrome import goes through the static `@atta/ui/components` alias, so `LibraryProvider`'s live-fetched value is carried but never read. If that ever changes — a future app-chrome component reaching for `useComponents()` instead of the alias, or the two config values genuinely diverging — the two paths would render different libraries in the same chrome tree, silently. This divergence is flagged, not resolved, here; whether to remove the now-unread live fetch or keep both is a call for whoever owns this invariant next, not a change bundled into whatever touched the pin.

- **Where the build-time id is plugged in (per-route resolution).**
  - `app/(app)/layout.tsx` feeds `CandidateShell` → `LibraryProvider` the CMS-resolved library id (`getProductConfig('herald').userInterface.library.id`, still a live call — see above). It deliberately ignores `user.library`. Wraps `/bulk-audit` and `/onboarding`.
  - `app/[username]/(owner)/layout.tsx` mirrors the `(app)` layout: feeds `CandidateShell` the same live CMS-resolved id, then renders the same `HeraldTopBar` + main. Wraps `/[username]/ui` and `/[username]/settings`. Two layouts, one library-resolution rule — adding a third app-chrome surface follows this template.
...
**Verification recipe:** set `user.library` to something other than the build-time library (e.g. `retro`). The main app chrome (`HeraldTopBar` on `/bulk-audit`, `/onboarding`) must stay on the build-time library; the owner editors at `/[username]/ui` and `/[username]/settings` must stay on the build-time library; only `/[username]` (the public profile) renders the user's choice. To confirm the build-time value itself, read `packages/ui/scripts/ui-library-pins.ts`'s `herald` entry — that is now the single source of truth for what a fresh build emits, independent of whatever `getProductConfig('herald').userInterface.library.id` currently returns.

**Theme choices are filtered by the selected library.** `/[username]/ui` partitions the theme list on the selected library: a neobrutalist library offers only themes flagged `neobrutalist`, and the soft libraries offer only the rest. **`brutal` is no longer selectable** — its `library` document was removed from the CMS in the same change (its vendored `installed/*` references tokens this repo never defined, so it had never rendered correctly), leaving `basic`, `retro` and `animate`. The partition logic still names it because its code remains until a follow-up removes it. Those libraries draw a hard border and offset shadow, so an un-tuned theme renders frameless there — and a neobrutalist theme under a soft library reads as a washed-out version of itself, since nothing draws the frame it was built around. Switching library also re-selects a compatible theme, since filtering the list alone would leave an already-chosen incompatible theme in place. Shared helpers `themesForLibrary()` / `isThemeCompatible()` live in `@atta/cms` so this editor and `tools/admin`'s themes page cannot drift apart. Consequence worth knowing: only two themes carry the flag today, so a candidate on `retro` sees a short list — that grows as themes are migrated, with no code change.

**Per-user profile theme (colors/typography) — a sibling concern to library, resolved differently.** Distinct from `user.library` above: a candidate can also pick a custom color theme (a `uiTheme` Sanity document — e.g. "Matrix Light") for their public profile via `/[username]/ui`. This is stored as `herald_profiles.theme_id` (a bare string, the `uiTheme` document's `_id` — the DB has no idea which Sanity project it belongs to) plus `color_scheme`/`font_sans` overrides, and rendered by `app/[username]/(profile)/page.tsx` into an attribute-scoped CSS override (`generateThemeCSS`) that wins the specificity battle against the global `NextWebShell` scheme block.

- **Same central-CMS-redirection rule as library resolution, easy to miss because the call site takes a generic client.** `getThemeById(client, themeId)` (`@atta/cms`) runs a raw GROQ lookup against whatever `SanityClient` it's handed — unlike `getProductUiConfig`/`getProductConfig`, it does **not** internally redirect to the central project. The caller must pass `createProductClient('attalabs')`, exactly as `/[username]/(owner)/ui/page.tsx` already does when listing themes to choose from.
- **The bug this closed:** `(profile)/page.tsx` was calling `getThemeById(<Herald's own client>, user.themeId)` — that client targets Herald's own per-product project (`e9gbd2d1`). Since the central-CMS migration, `uiTheme` documents live only in the central `attalabs` project (`l5n0n8nn`), so this always returned `null` and the user's chosen theme silently never applied on the public profile, on every refresh (not a caching issue — the route is `force-dynamic`). The `/ui` editor's live preview masked this during editing because it applies the theme object already in memory (fetched from the central project for the picker list), never calling `getThemeById` at all.
...
- Six tabs: Profile, Experience, Connections, API Keys, Herald Model, Account. (CV is folded into Experience; there is no separate CV tab.) Tabs are a single non-wrapping row that scrolls horizontally if a chunky library exceeds the width.
- **Container width is `max-w-3xl` on the Tailwind scale, not an arbitrary value.** It was `max-w-[700px]`, which the six-tab row overflowed under the chunkier libraries (retro/brutal add `border-2` plus padding to every trigger), so the tab strip scrolled while most of the viewport sat empty. Keep this on the scale — an arbitrary `max-w-[Npx]` is a hardcoded layout constant that no longer tracks the design system, and the width is a function of how wide the widest library renders six triggers, not of a chosen pixel count.
...
Profile audits (a recruiter auditing a published profile) run on the **profile owner's** vendor key; Bulk Audit runs on the **logged-in user's** vendor key. Same `getProviderKeys(db, clerkId)` path; only `clerkId` differs. Which vendor + model the audit dispatches against is the user's per-user selection (task 3b) — this governs **whose** key, orthogonal to **which vendor**. Publishing does not require a key; the audit tool renders only when the owner has at least one vendor key; the audit endpoint 503s/402s without one.
...
- **Single shape** — `{ job_description, username? | _test_profile_override? }` → returns a `MatchReport`. The Envoy single-profile audit (`EnvoyFlow.tsx`). Auth is open; the audit runs on the **profile owner's** stored BYOK key (DB-resolved by `username`). Test and Dani-fallback paths use the server `ANTHROPIC_API_KEY` on the YAML default model.
- **Batch shape** — `{ jd, candidates[] }` → returns `{ results: { username, report, error? }[] }`. The Bulk Audit (`BulkAudit.tsx`). Requires a Clerk session; runs on the **logged-in user's** stored BYOK key; 402 if missing. `Promise.all` fan-out across candidates (≤ 10).

Both shapes call the same `runSingleMatch(profile, jd, creds)` cell — the engine-backed unit of work. Everything around it is preserved verbatim from the original design:
...
- **`extractSignals` pre-fetch** (3s timeout, best-effort) still produces the GitHub signal evidence appended to `userPrompt`. A follow-up task will retire this pre-fetch by giving the auditor agent a GitHub tool declared in the YAML.
...
Rate limit: `src/proxy.ts` applies the Upstash 5/h per-IP cap to `POST /api/audit` (was `/api/match` before the endpoint unification). Side effect: batch calls are now in scope of the cap, but one batch call counts as one hit even when it audits up to 10 candidates internally.
...
- Per-audit (one-off) vendor + model override on the Bulk Audit surface. The per-user default has landed; a per-audit override (e.g. "run THIS batch on GPT-5 even though my default is Claude") is the natural next step but explicitly out of scope for V1.
- Per-key rate limit / cap on profile audits (abuse surface: strangers spend the owner's key budget).
- `herald.attalabs.dev` deploy verification.
- Auditor signal-gathering as a YAML-declared tool — retires the `extractSignals` pre-fetch.
