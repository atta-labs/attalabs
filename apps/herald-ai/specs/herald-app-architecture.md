...
- **Where the build-time id comes from (central CMS resolution).** Theme and library metadata live centrally in the `attalabs` (`l5n0n8nn`) Sanity dataset. In each product's local dataset (e.g. `heraldConfig`), `userInterface.theme` and `userInterface.library` are stored as simple string IDs. The `cms` package's resolver (`getProductUiConfig` / `getProductConfig`) intercepts those string IDs (or legacy references), fetches the fully populated structures from the central `attalabs` database, and reconstructs the standard `PortalUiConfig` shape so downstream consumers see the same object regardless of where the metadata physically lives. The build-time UI generator (`scripts/generate-ui.ts`) consumes this same resolver, so the alias `@atta/ui/components` → `packages/ui/generated/herald/components.ts` always matches what `getProductConfig('herald').userInterface.library.id` returns at runtime — the field is a `library.id` string regardless of central vs. legacy storage.
