...
- `HeraldAccountMenu` is a themed Sign-out button (not Clerk `UserButton`). Responsive: icon-with-text on `md+`, icon-only below. Its `Button` resolves **build-time** via `@atta/ui/components/button` on chrome surfaces (per the §4 D-035 rule), superseding its earlier `useComponents()` pattern. The one exception is the public-profile tree (`envoy-shell` under `[username]/(profile)`): that call site injects its `useComponents()`-resolved Button via the optional `ButtonComponent` prop, so the candidate's runtime library still drives it (ui-retro-contract-v1 f/u 2).
