...
- **Container width is `max-w-4xl` on the Tailwind scale, not an arbitrary value.** It was `max-w-[700px]`, which the six-tab row overflowed under the chunkier libraries (retro/brutal add `border-2` plus padding to every trigger), so the tab strip scrolled while most of the viewport sat empty. Keep this on the scale — an arbitrary `max-w-[Npx]` is a hardcoded layout constant that no longer tracks the design system, and the width is a function of how wide the widest library renders six triggers, not of a chosen pixel count.
