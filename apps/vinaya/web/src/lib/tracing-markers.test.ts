...
const readRoot = read('src/lib/repo-state/read-root.ts')
...
const WALKS = [
  ...
  {
    file: 'lib/repo-state/read-root.ts',
    label: 'CONFIG_DIR/REGISTRY_FILE',
    marker: `${constant(readRoot, 'CONFIG_DIR')}/${constant(readRoot, 'REGISTRY_FILE')}`,
    kind: 'file' as const
  },
  {
