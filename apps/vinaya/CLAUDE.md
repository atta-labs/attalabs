...
|---------|------|---------|--------|
| Web | `web/` | `@atta/vinaya-web` | Landing + Install (CLI command reference, `/install`) pages live; `/quick-start` — the five-step tutorial from nothing to a governed repo with an agent working inside it (`misc-hardening-v1` task 8, #682); Studio dashboard live locally at `/studio` (redirects to the `/the-studio` Portal page in production); methodology-doc browser at `/docs` (moved from `/studio/docs`, `vinaya-pages-v1` task 8, #568 — old path permanently redirects); site-wide TopBar on every route |
