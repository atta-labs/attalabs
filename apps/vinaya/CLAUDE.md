...
Every command's name/description/flags/status lives in a `commands.ts` `COMMANDS` registry — but in the **published** `@attalabs/vinaya-sources`, which the CLI's `printHelp()` and Portal's `/docs/cli` both render. The local `@atta/vinaya-sources` workspace member under `sources/` is a stale pre-extraction copy with no consumers in this repo; editing it changes nothing a reader sees. The published-lifecycle proof (`verify-published-lifecycle`) lives in `atta-labs/vinaya` alongside that registry, not here.
