...
| CLI | `cli/` | `@atta/vinaya-cli` | `vinaya help`/`version` router, hierarchical config loader, versioned `--json` envelope (`vinaya-cli-v1` task 1, #381); `vinaya studio` launches local Vinaya Studio against the current repo (`vinaya-studio-v1` task 5, #390). Remaining command logic (`init`, `check`, `doctor`, forge writes) not yet implemented. |
