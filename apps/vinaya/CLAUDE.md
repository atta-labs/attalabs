...
Every command's name/description/flags/status lives in a `commands.ts` `COMMANDS` registry — but in the **published** `@attalabs/vinaya-sources`, which the CLI's `printHelp()` and Portal's `/docs/cli` both render — though not from the same artifact. Portal resolves the package from `node_modules` when it builds, so an edit reaches `/docs/cli` on the next install; the CLI inlines it at publish time (the shipped bundle carries a literal `COMMANDS` array and never names the package), so `printHelp()` picks the edit up only on the next CLI publish. The local `@atta/vinaya-sources` workspace member under `sources/` was a stale pre-extraction copy with no consumers in this repo — deleted by the attalabs-adoption tranche alongside `packages/aeg-core`/`packages/aeg-forge-state`/`packages/aeg-types`. The published-lifecycle proof (`verify-published-lifecycle`) lives in `atta-labs/vinaya`, not here — in its own workspace package (`apps/cli`), not beside the registry.
