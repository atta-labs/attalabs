...
First mention of this file — created by `vinaya-cli-v1` task 1, per the brief's §2 correction (`vinaya-spec.md` already existed at dig time; this backlog file did not).
...
- ** decision-log back-fill — Resolved as.** See [](../../../docs/decisions-legacy.md) in `docs/decisions-legacy.md`, backfilled by the `fix/decision-backfill` PR. The Issue #381 comment's provisional "" label was superseded by a numbering collision with PR #517 (a different, unrelated decision that merged first and claimed) — ratified as instead.
