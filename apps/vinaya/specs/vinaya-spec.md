...
| `/studio/backlog` | Every open Issue with no `iteration:*` label, grouped by `project:<name>` label — closes the gap where unscoped Issues (e.g. #497) were invisible on every iteration-scoped Studio page | **Live** | Task 2 (#498) | Not yet applied — same functional-not-visually-polished status as the pages above |
