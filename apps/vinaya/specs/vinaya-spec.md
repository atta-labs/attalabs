...
| Studio dashboard — mounted at `/studio` (route-collision decision: root `/` is the marketing landing page) | The live derived-status viewer | **Live** | Task 1 (#388, PR #493) | Not yet applied — same functional-not-visually-polished status as the other pages below; two-tier nav (task 3, #500): site-wide TopBar (Home/Known Limits/AEG/Studio) on the root layout, Studio-only dashboard sidebar (Overview/Projects/Iterations/Backlog/Docs) at `studio/layout.tsx` |
