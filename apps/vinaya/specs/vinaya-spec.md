...
## `/how-it-works` page (`vinaya-pages-v1` task 4, #508) — supersedes `/aeg`

`apps/vinaya/web/src/app/how-it-works/` replaces the former `/aeg` methodology page (task 0b, #480) entirely — `/aeg`'s page, its 6 `_components/*`, and all of `src/lib/aeg/*` (the hand-rolled roles/rings/contracts parsers) are deleted; `/aeg` is now a bare `NextResponse.redirect(url, 301)` Route Handler. Where the old page read `aeg-root/**` through its own bespoke parsers, this page reads it through the shared, aeg-core-owned `deriveDiagramModel` (see "DiagramModel derivation" below) — one derivation library, N consumers, per D-087.
...
- **Groupings** (`_lib/groupings.ts`) — six interactive drill groups derived purely from the model: `ring0`/`ring1`/`ring2` (the `kind: 'ring'` nodes + their `gate`/`check` children), `action-github`/`action-internal` (the `kind: 'action'` nodes, split by `ACTIONS.crosses` — already exported from `@atta/aeg-core`, D-119's canonical source — cross-referenced by id, never a hardcoded list), and `actors` (`kind: 'role'` nodes). A static "the forge" hub frames the center — not a `DiagramNode` (the model has no dedicated substrate node kind), so it carries no data, same status as a section heading. Contracts render as chords between producer/consumer role positions once the `actors` band is drilled, resolved from the model's own `produces`/`consumes` edges.
...
- **Leaf panel** — exactly: label, a ring/kind tag, a `category`-or-`actorType` `Badge variant='outline'` (neutral, never color-coded — distinct from the diagram's own per-ring visual distinction, an unrelated concept), a render-state indicator when not `active`, the doctrine `summary`, and a "Read more" link. "Read more" resolves via `_lib/read-more.ts`: gate/check nodes link to `enforcement.md#L<sourceLine>`; role/contract nodes link to their own `aeg-root/roles|contracts/<id>.md` (the node's `label` *is* its doctrine frontmatter id, and every real file today is named `<id>.md`, verified at authoring time); action nodes get no "Read more" — `ACTIONS` lives in `packages/aeg-core/src/actions.ts`, not `aeg-root/**`, so there is no honest doctrine link to offer.
...
- **Render decision (`vinaya-pages-v1` task 4, #508):** `/how-it-works`'s leaf panel shows `category`/`actorType` as neutral `Badge variant='outline'` — never color-coded, and distinct from the diagram's own per-ring visual distinction (an unrelated, pre-existing concept). "Read more" links to a real GitHub blob line (`enforcement.md#L<sourceLine>` for gate/check nodes; `roles|contracts/<id>.md` with no line for role/contract nodes; omitted for action nodes, which have no doctrine markdown file). `DiagramModel.iteration` is passthrough task-lifecycle data, not per-check live status — so `/how-it-works` carries no "checks N/N" live pill, deliberately, even though the pre-existing design mockup this page's interaction pattern is drawn from shows one.
