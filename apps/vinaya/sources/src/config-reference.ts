...
export const CONFIG_REFERENCE: readonly ConfigField[] = [
  ...
  },
  {
    ...
    semantics: [
      ...
      'Resolved from a hardcoded `origin/main` ref via `git show` — never the PR’s own working tree, and never a caller-suppliable env var — so a PR that edits this field takes effect only for PRs opened after it merges, not for itself. Adding someone to `principals` therefore still requires an existing principal’s approval, same as any other change.'
    ],
