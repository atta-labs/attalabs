...
export const COMMANDS: readonly Command[] = [
  ...
  },
  {
    ...
    details: [
      ...
      'The branch is read with `git symbolic-ref`, which yields nothing when HEAD is detached or unborn — deliberately, since `git rev-parse --abbrev-ref HEAD` reports the literal string `HEAD` in that state, and treating that as an ordinary non-task branch would take the relaxed path and skip sections. An unresolvable branch is fail-closed: every configured section is enforced.'
    ],
    status: 'shipped'
  },
  {
