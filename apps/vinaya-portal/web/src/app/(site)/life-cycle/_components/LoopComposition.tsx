...
import { type NounId, NounMark, NOUNS } from './StageGlyph'
 ...
 * design's.
 *
 * Does NOT mount `MarkDefs` — on `/life-cycle` every stage section also
 * `<use>`s the same `#v-*` defs, so the page mounts it exactly once, at
 * page level. (`/start/overview`, this component's original and only
 * consumer, used to be the sole reason it self-mounted; that consumer is
 * gone.) Any caller must ensure `MarkDefs` is mounted somewhere on the page.
 */
