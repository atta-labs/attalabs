 ...
 * resolves against it. `/life-cycle/page.tsx` mounts it once, at page level
 * — every `StageDiagram` and `LoopComposition`'s own noun marks `<use>` into
 * that single mount, rather than each mounting its own (the old per-route
 * `/start/*` pages each mounted their own copy through `StagePage`; that
 * per-section mounting is the trap this single-page layout must not repeat).
 * ---------------------------------------------------------------------------
 */
