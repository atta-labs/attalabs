...
export default async function HowItWorksPage() {
  ...
  // Two distinct hrefs per node: `readMoreHrefs` is the in-app rendered doc
  // (only exists for kinds `readMoreTarget` gives a `docRoute` — gate/check/
  ...
  for (const node of model.nodes) {
    ...
    if (node.kind === 'role' || node.kind === 'contract') {
      ...
      if (!surfacedSlugs.has(slug)) {
        throw new Error(
          `readMoreTarget for ${node.kind} node "${node.label}" resolves to doc slug ` +
    ...
    readMoreHrefs[node.id] = target.docRoute
  }
  ...
  return (
    ...
    <div className='w-full lg:h-full lg:overflow-hidden'>
      <DiagramExplorer
        ...
        readMoreHrefs={readMoreHrefs}
        viewSourceHrefs={viewSourceHrefs}
      />
    </div>
  )
}
