// V2 Step 3.5 — A0S vs B0S judge comparison (Sonnet replication of Step 2).
//
// Loads A0S runs from v2_baseline_runs and B0S runs from v2_orchestration_runs
// (filtered by model_id='claude-sonnet-4-6'), calls /api/benchmark/v2-judge for each pair,
// results persist to v2_judge_results with comparisonType='baseline-vs-vada-sonnet'.
//
// Usage (from apps/vada-ai/web/):
//   bun --preload ./scripts/preload-server-only.ts scripts/bench/v2/compare-a0-b0-sonnet.ts
//   bun --preload ./scripts/preload-server-only.ts scripts/bench/v2/compare-a0-b0-sonnet.ts T1
//   bun --preload ./scripts/preload-server-only.ts scripts/bench/v2/compare-a0-b0-sonnet.ts --position-mode balanced
//
...
const DESC = {
  A0S: 'A0S naive single-shot baseline on Sonnet (terse, direct)',
  B0S: 'B0S V1 Vāda full deliberation workflow on Sonnet (orchestration-alone)'
}

function slotAssignment(runIndex: number, mode: PositionMode): { slotA: 'A0S' | 'B0S'; slotB: 'A0S' | 'B0S' } {
  const a0First = mode === 'standard' ? runIndex % 2 === 0 : runIndex % 2 === 1
  return a0First ? { slotA: 'A0S', slotB: 'B0S' } : { slotA: 'B0S', slotB: 'A0S' }
}
...
async function callV2Judge(params: {
  ...
  const res = await fetch(`${BASE_URL}/api/benchmark/v2-judge`, {
    ...
    body: JSON.stringify({
      ...
      comparisonType: 'baseline-vs-vada-sonnet'
    }),
...
async function main() {
  ...
  console.log(`\n${hr('═')}`)
  console.log('  V2 Step 3.5 — A0S vs B0S Judge Comparison (Sonnet)')
  console.log(hr('═'))
  ...
  for (const question of questions) {
    ...
    for (let runIndex = 0; runIndex < V2S_RUNS_PER_CONFIG; runIndex++) {
      ...
      const existing = await getExistingV2JudgeResult(question.text, systemADesc, runIndex, 'baseline-vs-vada-sonnet')
      if (existing) {
      ...
      const a0SRun = await getV2BaselineRun(question.id, 'A0S', runIndex)
      const b0SRun = await getV2OrchestrationRunForModel(question.id, runIndex, V2S_JUDGE_MODEL_ID)

      if (!a0SRun || !b0SRun) {
        console.log(
          `  [run ${runIndex}] ✗ Missing run(s) — run baseline-ceiling-sonnet.ts + orchestration-alone-sonnet.ts first`
        )
        totalErrors++
        continue
      }

      const responseA = slotA === 'A0S' ? a0SRun.responseText : (b0SRun.conclusionText ?? '')
      const responseB = slotB === 'B0S' ? (b0SRun.conclusionText ?? '') : a0SRun.responseText
  ...
  console.log(`\n${hr('═')}`)
  console.log('  Comparison run complete (Sonnet)')
  console.log(hr('─'))
