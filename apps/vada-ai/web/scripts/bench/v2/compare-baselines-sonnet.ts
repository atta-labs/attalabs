// V2 Step 3.5 — A0S vs A1S judge comparison (Sonnet replication of Step 1).
//
// Loads A0S and A1S runs from v2_baseline_runs, calls /api/benchmark/v2-judge
// for each pair, results persist to v2_judge_results with comparisonType='baseline-vs-baseline-sonnet'.
//
// Usage (from apps/vada-ai/web/):
//   bun --preload ./scripts/preload-server-only.ts scripts/bench/v2/compare-baselines-sonnet.ts
//   bun --preload ./scripts/preload-server-only.ts scripts/bench/v2/compare-baselines-sonnet.ts T1 B3
//   bun --preload ./scripts/preload-server-only.ts scripts/bench/v2/compare-baselines-sonnet.ts --position-mode balanced
//
...
// Descriptions encode variant identity so analysis scripts can normalize without
...
function slotAssignment(runIndex: number, mode: PositionMode): { slotA: 'A0S' | 'A1S'; slotB: 'A0S' | 'A1S' } {
  const a0First = mode === 'standard' ? runIndex % 2 === 0 : runIndex % 2 === 1
  return a0First ? { slotA: 'A0S', slotB: 'A1S' } : { slotA: 'A1S', slotB: 'A0S' }
}
...
async function callV2Judge(params: {
  ...
  const res = await fetch(`${BASE_URL}/api/benchmark/v2-judge`, {
    ...
    body: JSON.stringify({
      ...
      comparisonType: 'baseline-vs-baseline-sonnet'
    }),
...
async function main() {
  ...
  console.log(`\n${hr('═')}`)
  console.log('  V2 Step 3.5 — A0S vs A1S Judge Comparison (Sonnet)')
  console.log(hr('═'))
  ...
  for (const question of questions) {
    ...
    for (let runIndex = 0; runIndex < V2S_RUNS_PER_CONFIG; runIndex++) {
      ...
      if (!runA || !runB) {
        console.log(`  [run ${runIndex}] ✗ Missing baseline run(s) — run baseline-ceiling-sonnet.ts first`)
        totalErrors++
        continue
      }
