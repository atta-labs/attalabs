...
On May 4, 2026, the hosted MCP server shipped, which required server-side decryptable provider keys to call provider APIs on the user's behalf. A brief intermediate architecture tried to maintain both stores with synchronization, but the UX implications — especially the lock-icon "Sign out / Forget device" affordances on the deliberate page that no longer reflected reality once the server held a copy of every key — surfaced a sync bug within minutes of feature use. The single-source-keys reversal demoted IndexedDB from canonical key storage and made server-side `user_provider_keys` the single source of truth.
