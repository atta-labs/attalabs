...
## D-127 — The page that renders the harness is "The Harness", served at `/the-harness`; `/how-it-works` permanently redirects
...
**Decision:** The route group `(site)/how-it-works/**` moves to `(site)/the-harness/**` — renderer, `_lib`, and `_components` unchanged; the diagram, `deriveDiagramModel`, `load-diagram.ts`, and `read-more.ts`'s `/docs/**` targets are untouched (renaming the page is not renaming the model). `/how-it-works` becomes a permanent (308) redirect to `/the-harness`, slug-preserving, mirroring the `/studio/docs` → `/docs` redirect shape (#587); it exists for external inbound links only. Every in-app link and every nav label naming the page is updated in the same change — the site TopBar entry and the Home-page/Portal nav now read **The Harness → `/the-harness`**; the redirect is never the fix for an in-app link. `role_id`/`contract_id`, `enforcement.md`'s parse contract, and every `aeg-root/**` file are out of scope — `AEG` survives repo-internally exactly as D-123 left it.

**Alternatives rejected:** Keeping `/how-it-works` as the route and only relabeling the nav — rejected: the URL is the citable surface, and a public page advertising a genre-named path undercuts the rename. Deleting `/how-it-works` outright with no redirect (as D-123 did for `/aeg`) — rejected: `/aeg` was a retired *brand*, whose one-hop survival was the objection; `/how-it-works` is a plain descriptive slug of a page that still exists at a new address, so a permanent redirect preserves any already-shared link without republishing a retired name. Renaming the page's internal chrome (breadcrumb, `<title>`) in the same change — deferred: this task renames the URL and the nav entry only, keeping the renderer byte-identical per its brief; aligning the page's own "How it works" breadcrumb/title to "The Harness" is a follow-up, not banked here.

**Consequences:** External links to `vinaya.attalabs.dev/how-it-works` now 308 to `/the-harness` rather than resolving directly; the page had no sub-paths, so one redirect stub covers every inbound URL. `apps/vinaya/web/src/app/(site)/the-harness/_lib/load-diagram.ts` retains a `/how-it-works` mention in its build-time docstring — left untouched because the file is out of this task's edit surface (moved, not edited); it is a comment, not a live href or visible string. The page's own breadcrumb and browser `<title>` still read "How it works" (renderer kept byte-identical) — a visible inconsistency with the new nav label, flagged for a follow-up. Reversing this decision is a second directory move plus flipping the redirect; nothing structural depends on the name.
