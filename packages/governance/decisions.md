...
**Date:** 2026-07-11
**Status:** ACTIVE
**Type:** 2 (reversible via a superseding D-entry; enforcement is a new CI gate, not new infrastructure)
...
**Decision:** No on-disk file may be a live state store going forward. Concretely: (1) `aeg-root/iterations/aeg-drift-prevention-v1.md` is deleted — the null-check `mergeTaskEdgesFromFile` already applies for iterations with no file (`herald-hardening-v1`, `vinaya-studio-v1`, `vinaya-cli-v1`) now also applies here; (2) `developer.md` items 3/6/7 and `brief-authoring/SKILL.md`'s two Dig preconditions and branch-ID paragraph now query the forge directly instead of reading a topology file; (3) a new CI gate, `packages/aeg-core/bin/check-no-disk-state.ts` (predicate: `isNewDiskStateFile`), fails a PR that adds or edits a live `aeg-root/iterations/*.md` file (other than `README.md`) or adds any new `*.tokens.md` file anywhere in the repo, wired into `forge-lifecycle.yml`'s `aeg-gate-suite` job. `aeg-root/iterations/completed/**` (the Studio archive, 14 files) and `vada-production-v1`'s tracked-exception file are explicitly excluded from both the deletion and the new gate — reversing either is a separate, deliberate Type 1 decision (a Studio forge-derivation rebuild for the former; already covered by D-113 for the latter), not a mechanical cleanup.

**Alternatives rejected:** Deleting `completed/**` per Issue #512's literal Part B text — rejected; Studio's Archive view has no fallback for it today, and doing so without a rebuild would break a live product feature. Filed as a follow-up Issue instead (Part B', referenced in the closing PR) rather than silently deferred. Making the new CI gate retroactively flag existing `completed/**` files — rejected: the gate's job is to stop new state-file creation, not to re-relitigate an already-tracked, deliberately-excluded archive on every unrelated PR.
