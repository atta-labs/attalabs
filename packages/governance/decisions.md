...
## D-117 — GitHub is the sole state machine; no file on disk represents work state; enforced by `check-no-disk-state`

**Date:** 2026-07-11
**Status:** ACTIVE
**Type:** 1
**Lock:** YES
**Conforms-to:** D-055, D-110, D-113
**Authored by:** Developer (task `forge-sole-state` 1)
**Ratified by:** Principal (in-task brief)

**Context:** D-113 (`aeg-forge-state-v1` task 7, #431) completed the forge-native cutover for every active iteration except `vada-production-v1`, and explicitly kept `aeg-root/iterations/completed/**` and every `.tokens.md` out of scope, citing `iterations/README.md` §11's "archived iterations are durable history, never deleted." `vada-production-v1`'s 9 grandfathered Issues were subsequently backfilled and its file deleted in #486. Since then, disk-state-as-source-of-truth kept resurfacing as a live failure mode: an agent reading a stale/absent `aeg-root/iterations/<slug>.md` and concluding a forge-native task "doesn't exist," when the forge said otherwise. This task audited every reference to on-disk iteration state across the repo (code, CI, role docs, contracts, skills, Studio/Vinaya loaders) and found the remaining residue was smaller, and more structural, than assumed: `@atta/aeg-forge-state`'s `deriveIterationFromForge`/`parseRationaleDeps` already fully reconstruct a task's identity, projects, and `dependsOn`/`conflictsWith` edges from its Issue alone, and `listActiveIterationSlugs` (new: `listArchivedIterationSlugs`) already enumerate iterations from Milestones, not directories — the topology file's function was already fully superseded, just not yet deleted or documented as such.

**Decision:** GitHub (Issues, Milestones, PRs, PR/Issue comments) is the sole state machine for AEG work state — what iterations exist, what tasks exist, their tier/project/dependencies, and what's dispatchable/in-flight/in-review/done. No file on disk may represent any of this, with two closed, non-growing, explicitly-named exceptions this task could not resolve without inventing new forge infrastructure or destroying irrecoverable data:
1. **A small, fixed set of pre-Milestone-era legacy iterations** (`GRANDFATHERED_LEGACY_PATHS`, `packages/aeg-core/src/no-disk-state-gate.ts`) with no Milestone, open or closed, to forge-derive their archival status or topology from. Their `completed/*.md` files (and, where applicable, `.tokens.md` siblings) remain the only record.
2. **The per-iteration token/cost ledger** (`iterations/<name>.tokens.md`, D-071). Every role's token report already lands on the forge (PR body, PR/verdict comment) except two: the Archivist's own `<task-id>: archive` row (no PR of its own) and the Planner's iteration-wide `planning` row (no reliable forge-only attribution to one task, confirmed live building the Studio's live-read path, `iterations/README.md` §12). Resolving this needs a follow-up decision — a forge-native home for those two row types, or accepting the gap — not invented unilaterally here.

Both exceptions are enforced **closed** (cannot silently grow) by a new blocking CI gate, `check-no-disk-state.ts` (pure evaluator `checkNoDiskState`, `packages/aeg-core/src/no-disk-state-gate.ts`), wired as a step of the AEG gate suite (`.github/workflows/forge-lifecycle.yml`). It fails the build on: any live `aeg-root/iterations/*.md` other than `README.md` (no exceptions — every active iteration is forge-native), or any `aeg-root/iterations/completed/*.md` / `*.tokens.md` file not already in the fixed allowlist.

Deleted: the live topology file `aeg-drift-prevention-v1.md` (Milestone #17 confirmed open) and the 2 `completed/` files with a corresponding closed Milestone (`aeg-forge-state-v1.md`, `aeg-review-gate-v1.md`) — 9 other `completed/` files had no Milestone at all (pre-D-110) and were restored after an initial over-broad deletion attempt proved they had no forge-derivable replacement; see this task's PR body for the full audit table. Migrated to forge-derived (closed-Milestone) reads, replacing file-based ones: `apps/aeg/web/studio` and `apps/vinaya/web`'s `aeg-fs/read-root.ts` archived-iteration loader; `verify-coherence.ts`'s general (unscoped) sweep enumeration; `stale-blocker.ts`'s active-iteration enumeration; `apps/aeg/web/studio/scripts/smoke-forge.ts`'s closed-Milestone resolution. Rewritten to reference the forge instead of a topology file: `roles/developer.md` entry-gate items 3/6/7, `contracts/brief-developer.md`, `contracts/iteration-archivist-planner.md`, `contracts/archivist-iteration-archivist.md`, `roles/planner.md` item 8, `skills/brief-authoring/SKILL.md` preconditions, `skills/aeg/SKILL.md`, `coordination.md`, `process.md`, `state-machine.md` (§3, §15b, §15d, new §15e), `enforcement.md`'s ring-1 registry, and `iterations/README.md` itself (§1, §4, §9, §11 rewritten; §12 token-ledger doctrine deliberately left unchanged, per exception 2 above).

**Alternatives rejected:** Deleting `completed/**` and `*.tokens.md` unconditionally, as the dispatched brief's premise assumed — rejected once live verification (checking actual GitHub Milestone state, not the doc's claims) showed only 2 of 11 `completed/` iterations have any Milestone at all; deleting the other 9 would have destroyed the only record of their topology with no forge path to recover it, exactly the Section 10 stop condition ("deleting a file breaks a loader with no forge path yet") the brief itself named. Silently keeping the guard scoped to match only what could safely be deleted, without recording why — rejected; the gap is real and durable enough (a fixed pre-Milestone-era set, a real token-ledger doctrine hole) to need its own decision entry, not a buried code comment. Inventing a forge-native home for the Archivist/Planner token rows unilaterally (e.g. an Issue comment convention) — rejected as exceeding this task's brief; it is a real design choice with its own tradeoffs, named here as a follow-up, not decided here.

**Consequences:** Any future PR that tries to reintroduce a live iteration topology file, or a new `completed/`/`.tokens.md` file outside the fixed allowlist, fails CI immediately (`check-no-disk-state`), regardless of intent — "just this once, for convenience" is now structurally impossible, not just discouraged. The two named exceptions are follow-up work, not closed by this entry: (a) backfilling a Milestone for each grandfathered legacy iteration, mirroring `vada-production-v1`'s D-078 backfill, would let each be deleted and the allowlist shrink; (b) the token-ledger doctrine gap needs a Type 1 Principal decision on whether to give Planner/Archivist rows a forge-native home or accept the loss. Neither blocks this entry from being ACTIVE now — the guard already enforces the intended invariant everywhere it currently has a forge answer.
