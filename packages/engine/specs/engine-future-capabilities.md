...
1. **`steps[]`** — ordered, heterogeneous, alongside `rounds[]` rather than replacing it.
...
4. **`type: step`** — a mechanical node with no LLM turn. Merge, tag, and `npm publish` are `gh`/shell calls with no model in them, and today every node requires an agent with a `system_prompt`.
...
- `PlanNodeKind` has **no production reader** — `compile-flow.ts` writes it, tests assert it, nothing else consumes it. `PlanNodeRole` is what the adapter routes on. A new node type must therefore land on the role, not only the kind; adding a kind alone is a render label that executes nothing.
