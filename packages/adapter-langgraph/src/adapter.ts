...
/**
 * Estimate input token count/cost for a text blob against a given model,
 * without running it. ~4 chars/token approximation, input-rate only.
 * Returns `costUsd: null` for a `modelId` absent from `PRICING`.
 */
export function estimateInputCost(text: string, modelId: string): { tokens: number; costUsd: number | null } {
  const tokens = Math.ceil(text.length / 4)
  const pricing = PRICING[modelId]
  const costUsd = pricing ? (tokens * pricing.input) / 1_000_000 : null
