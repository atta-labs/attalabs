...
> **vinaya** — Vinaya: the npm-distributed reference implementation of AEG, installed into any GitHub repository. This row's own folder, `apps/vinaya`, now holds only this project's specs — the `sources` workspace member (the StateSource adapters — forge-backed primary, file-backed transitional) was a stale pre-extraction copy deleted alongside `packages/aeg-core`/`packages/aeg-forge-state`/`packages/aeg-types` in the attalabs-adoption tranche; the web surface it used to hold split into two sibling apps, registered below as their own rows (`vinaya-portal`, `vinaya-studio`), once `apps/vinaya/web` was deleted (task 4, #886) with both replacements proven on `main`. The CLI (the `vinaya` command — init/check/pr/issue/doctor/upgrade/eject/demo/waiver) was extracted to the standalone `atta-labs/vinaya` repository and is installed here from npm as `@attalabs/vinaya`; attalabs is an ordinary adopter of the published package. The registry row deliberately precedes the code. "Vinaya is the reference implementation of AEG": AEG stays the model's name; Vinaya is the tool. `aeg-root/` here now holds only this repo's own tranche history (`tranches/**`) and three operational-prose files the published package doesn't bundle — the portable doctrine text itself resolves through the installed `@attalabs/vinaya` package (`vinaya doctrine`), not a local copy. Pāli name by elective aesthetic. `apps/vinaya` carries no `-ai` suffix (meta/infra convention).
