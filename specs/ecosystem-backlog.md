...
- ~~**Ledger-ownership fix (flagged 4× by reviewers).**~~ **Dispatched** — aeg-governance-hardening #266 (D-071): no role writes its own ledger row on a task branch; the Archivist records all rows post-merge at close-out.
- ~~**Reviewers must write nothing to disk (dirty-`main` incident).**~~ **Dispatched** — aeg-governance-hardening #266: reviewers/security write only PR comments; dispatched agents run only in an isolated worktree, never the main checkout. (The "make a dirty `main` structurally impossible" stretch goal remains open — hook-level enforcement was explicitly out of scope for that task.)
- ~~**`executor-protocol` should chain to `roles/developer.md`.**~~ **Dispatched, then revised** — aeg-governance-hardening #266: the first pass had `.claude/skills/executor-protocol/SKILL.md` open with "read `aeg-root/roles/developer.md` first," which Principal review found violates the one-way knowledge law (host repo must never reference AEG — D-072). R1 reverted that line; the chain now lives in the brief itself (`aeg-root/skills/brief-authoring/SKILL.md`'s mandatory Role-chain preamble), never in a host-repo artifact.
- **Mechanical D-072 boundary check** — a `verify-docs` check that greps the diff for monorepo→AEG references outside the sanctioned crossings (AEG-owned views, Cetana, historical records, planning-seam backlogs) and fails the gate. Today the one-way knowledge law (D-072) is doc-discipline only; this would make it mechanically enforced. Not built in this task — noted as future work.
