...
**Last updated:** July 4, 2026 (Vinaya program planned: vinaya-cli-v1 + vinaya-studio-v1 registered, dispatch frozen; gov-hardening task 29 added; D-083–D-109 PENDING)
...
**The Vinaya program is planned and frozen (2026-07-04):** `vinaya-cli-v1` (7 tasks, #381–#387) and `vinaya-studio-v1` (3 tasks, #388–#390) are fully planned — topology files in `aeg-root/iterations/`, rationales on the Issues, decisions D-083–D-109 PENDING ratification (see `ratification-queue.md`). Vinaya = the npm-distributed reference implementation of AEG (`apps/vinaya`: cli + web/Studio + sources; registry row added). Task 29 (#380, waiver authentication — D-097, supersedes D-080 on ratification) was added to `aeg-governance-hardening`, serialized after in-flight task 25. **Nothing in the Vinaya iterations dispatches until `aeg-governance-hardening` fully archives and the Principal triggers dispatch.** Cetana is retired as a decision (D-095) but its code/specs stay in place — harvest happens inside vinaya-cli-v1; deletion + docs sweep ride a later iteration gated on "Vinaya CLI + Studio work 100%."
