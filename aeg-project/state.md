...
**Last updated:** July 6, 2026 (`aeg-governance-hardening` iteration complete — 39/40 tasks merged, archived to `completed/`; retrospective appended to `lessons.md`; Vinaya dispatch trigger now open — see below)
...
**`aeg-governance-hardening` is complete** (39 of 40 planned tasks shipped July 2–6: topology rows 1–33/35/36 plus sub-rows 5a–5d; task 34 consolidated into task 31, not abandoned; archived to `aeg-root/iterations/completed/aeg-governance-hardening.md`; retrospective in `lessons.md`). It finished D-069's role-seam gates (Planner→Brief R1, Brief→Developer brief-validation), drove the doc-owners audit + 5 promoted stale-doc fixes, mechanized `verify-dispatch`/`verify-task` into the pre-push and PR-open paths so the two highest-value gates can no longer be skipped, closed the plan-PR race and several coherence-oracle false positives, cut CI cost roughly in half via diff-scoping and job consolidation, replaced prose-described artifact shapes with structured templates + anchored gate-read fields, and closed the task 29/D-097 waiver-authentication task (actor-verified `waiver:docs` label supersedes the agent-forgeable `Doc-waiver:` grammar) — the iteration's last task, unblocked by the Principal's out-of-band ratification of D-097. Prioritized ahead of `vada-production-v1` by explicit Principal decision (2026-07-01) — Vāda work resumes once the Principal decides; nothing about that decision is made by this iteration's close-out. **`aeg-studio-cleanup` is complete** (5 of 5 tasks shipped July 2–3: #287/#290/#291/#292/#331; archived to `aeg-root/iterations/completed/aeg-studio-cleanup.md`; curated AEG Studio's UI surface — added the Documentation Coherence reference doc, removed the board/graph iteration views and the dead topbar link, fixed in-flight/in-review badge contrast within the existing token vocabulary, and wired `/docs` nav to the D-079 surfaced-doc manifest so execution-state files no longer leak into it. See the retrospective in `lessons.md`). **`aeg-consolidation` is complete** (4 of 4 tasks shipped July 1–2: #263/#264/#220/#265; archived to `aeg-root/iterations/completed/aeg-consolidation.md`; homed the coherence-check engine in `@atta/aeg-core`, fixed the CI≠local drift, re-armed the coherence gate as blocking, added the docs coherence gate — D-079. See the retrospective in `lessons.md`). **`aeg-coherence-v1` is complete** (6 of 9 tasks shipped June 25–July 1; archived to `aeg-root/iterations/completed/aeg-coherence-v1.md`; its 5 unbuilt tasks moved to `aeg-consolidation` + `aeg-governance-hardening` via D-070, not abandoned). **`vada-production-v1` is parked, not paused-and-forgotten** — 6 done / 15 open tasks, `Lifecycle: active`, untouched this session; two of its issues (#180, #182) read `Incoherent` (closed `COMPLETED` with no linked merged PR) and need a provenance fix or reopen — flagged, not yet actioned. **herald-agents-v2 is complete** (8 tasks, June 18–29; iteration archived to `aeg-root/iterations/completed/herald-agents-v2.md`; #234 open bug — prod ANTHROPIC_API_KEY likely expired, Principal to disposition).

**The Vinaya program is planned and frozen (2026-07-04):** `vinaya-cli-v1` (7 tasks, #381–#387) and `vinaya-studio-v1` (3 tasks, #388–#390) are fully planned — topology files in `aeg-root/iterations/`, rationales on the Issues. **D-097 (waiver authentication) ratified 2026-07-06** — carved out ahead of the rest of the batch (see `ratification-queue.md` D-097 carve-out note); implemented by `aeg-governance-hardening` task 29 (#380), now merged (PR #424). The remaining decisions D-083–D-109 minus D-097 (26 entries) stay PENDING ratification (see `ratification-queue.md`), gated on the Strategist-mode `aeg-root/` layout decision. Vinaya = the npm-distributed reference implementation of AEG (`apps/vinaya`: cli + web/Studio + sources; registry row added). **`aeg-governance-hardening` has now fully archived (2026-07-06) — the one precondition this program's dispatch was waiting on is satisfied. Dispatch still requires an explicit Principal trigger** (a deliberate "what's next" call this close-out does not make) **and the remaining 26 PENDING decisions are a separate gate the layout question above still blocks.** Cetana is retired as a decision (D-095) but its code/specs stay in place — harvest happens inside vinaya-cli-v1; deletion + docs sweep ride a later iteration gated on "Vinaya CLI + Studio work 100%."

Check `gh issue list --label "iteration:aeg-governance-hardening" --state open` and `gh issue list --label "iteration:vada-production-v1" --state open` for live task status.
...
**July 2–3, 2026 — aeg-studio-cleanup iteration complete.** Curated AEG Studio's UI surface, 5 task PRs: #313, #304, #314, #322, #346. Added a new canonical "Documentation Coherence" reference doc (`aeg-root/documentation-coherence.md`) explaining who reads/writes docs at each AEG role seam. Removed the per-iteration board and dependency-graph views (table-only task list), then removed the now-dead "Dependency graph" topbar link that survived that removal. Fixed the `in-flight`/`in-review` status badges to be visually distinct and lighter/brighter, using only the existing `primary` semantic token (a prior attempt to add a new `--info` token was rejected by the Principal and superseded). Wired Studio's `/docs` nav loader to the D-079 surfaced-doc manifest, removing the hardcoded `Iterations` nav section outright so active iteration topology files, token ledgers, and other execution-state artifacts no longer leak into the canonical docs tree; `iterations/README.md` re-homed to Overview via frontmatter as the one genuinely canonical exception. Along the way, task 5 hit and confirmed the fix for D-080 (pre-push doc-coverage gate's first-push gap, closed by a commit-trailer fallback). See the retrospective in `lessons.md`.
