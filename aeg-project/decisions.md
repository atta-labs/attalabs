...
4. **CI keeps running the identical checks** (same aeg-core code) as a backstop for non-hook writers (GitHub UI edits, other tooling). A red CI on a gated rule now signals a hook bug, not an agent failure.
