...
3. **Forge-lifecycle gates (T2 / #217).** A `Closes #N` pre-merge CI gate (Layer 1 — prevent); `verify-coherence` wired as a blocking required check covering A1/A2/A3/L1/L2 (Layer 2 — detect), with **A2 hard-blocking** the next in-iteration merge (close-out/archiving back-pressure) and **L1/L2 advisory** (avoids the last-task deadlock).
