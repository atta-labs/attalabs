...
## June 15, 2026 — Polymorphic audit inputs in Bulk Audit (task 5)

### Herald
- **Task 5 (#92)** — Bulk Audit's JD and CV slots are now polymorphic. Each JD slot is `text | url` and each CV slot is `text | markdown (.md) | pdf | profile` (a published Herald username). All input kinds normalise to the text the audit cell already consumes — `/api/audit`, `runSingleMatch`, the engine, `herald-auditor.yaml`, and the tool-execution path are unchanged (Task 7b's surface). New module `src/lib/audit-input/` holds the pure resolution layer: `resolve.ts` (`resolveJdInput`, `resolveCvInput`), `url-fetch.ts` (SSRF-safe URL fetch + HTML-to-text extraction), `client.ts` (thin client helper), and `types.ts` (`JdInput`/`CvInput`/`ResolvedJd`/`ResolvedCv`). A new authenticated endpoint `POST /api/audit/resolve-input` accepts JSON (`text`/`url`/`profile`) and multipart (`.pdf`/`.md`, 5 MB cap) and returns `{ resolved }` — PDFs are parsed with `unpdf` (the same library `/api/admin/parse-cv` uses); markdown is the file body as-is; profile lookup goes through the existing `getUserByUsername` and 404s when the profile isn't published. JD URL fetch validates the protocol (`http(s)` only) and rejects loopback / RFC1918 private / link-local / cloud-metadata / IPv6 unique-local / `.localhost`/`.local`/`.internal` hosts before hitting the network, caps the response at 1 MB and 8 s, and strips `<script>`/`<style>`/`<noscript>` blocks before tag-stripping the HTML. New components: `JdInputControl.tsx` (Tabs: Paste text / URL) and `CvInputControl.tsx` (Tabs: Paste text / .md / .pdf / Profile, with key-based file-input remount on kind switch). `BulkAudit.tsx` now lists JDs and Candidates symmetrically with per-slot add/remove and kind tabs (replacing the prior newline-separated usernames textarea), resolves every filled slot in parallel before fan-out, then runs the existing N×M matrix. **Audit wiring per CV kind:** `profile` → existing batch shape `{ jd, candidates: [username] }` (end-to-end, preserves per-owner BYOK + per-owner rate limit); `text` / `.md` / `.pdf` → existing single-shape with the resolved text packed into the `_test_profile_override` field (the only existing non-DB profile seam) — this works today **but spends the server's `ANTHROPIC_API_KEY` env fallback rather than the logged-in user's BYOK**. A follow-up (Task 7b or a separate ticket) should promote the seam to a production-named field that resolves logged-in user's credentials, so text-CV audits charge the user's BYOK rather than Herald's env key. 16 new unit tests in `tests/audit-input-resolve.test.ts` cover URL SSRF validation (private IPv4/IPv6, cloud metadata 169.254.169.254, internal hostnames, non-`http(s)` schemes), HTML text extraction (script/style stripping, entity decoding, paragraph breaks), JD text validation (≥20 chars), and CV text/markdown/pdf pass-through (≥50 chars). Tier 1 — Herald-only, no `@atta/ui` edits, no engine/adapter/`@atta/db` edits, semantic tokens only. Closes #92.
