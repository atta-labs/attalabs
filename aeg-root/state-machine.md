...
For the prose walkthrough (thirteen phases), see `process.md`. For the iteration/task model, see `iteration-model.md`. For role-specific instructions, see `roles/`. For the role-seam contracts, see `contracts/`.
...
**A task is a forge Issue, and its status is derived, never stored** (see Section 2, Class 2, and `iteration-model.md` §3). No role writes a status field; transitions are facts about the forge (branch exists, PR open, review decision, merged).
...
4. PM docs (iteration files) plus per-project pinned state/lessons Issues — non-derivable operational facts / plan topology (active status derived from forge)
...
- **Dispatch gates** (depends-on merged / no conflicting PR open) — read from the forge and complied with; mechanical enforcement arrives when an automation tool runs dispatch (`iteration-model.md` §8).
...
- **No execution metadata in the iteration file; no dynamic conflict scanner** — the two anti-regression rules (`iteration-model.md` §9); trusted discipline, flagged by the Archivist drift cron and the Planner's gates.
