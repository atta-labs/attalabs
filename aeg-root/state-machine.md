...
| **Iteration topology** (forge-native: task→issue map + edges live on Milestones/Issues, no file) | Approves the plan (Issues + Milestone) | Writes (Planner mode) at plan time — cuts Issues, sets `depends-on`/`conflicts-with` in each Issue's "Dependency rationale" body section | — | Flags a live `aeg-root/iterations/*.md` reappearing (`check-no-disk-state`, blocking — forge-sole-state task 1) |
...
The seam between **iterations** (the plan — forge Milestones + `iteration:<slug>`-labeled Issues, and, for a small closed set of legacy pre-Milestone iterations, `aeg-root/iterations/completed/*.md`) and **the forge** (GitHub Issue state / PR merge events). D-067 makes this seam deterministically verifiable: a stateless oracle that detects drift without LLM calls, runnable in CI.
...
- **Prior-iteration archival** resolves "the previous iteration for project P" as any other active iteration — candidate slugs come from `listActiveIterationSlugs` (open Milestones, `otherActiveIterationSlugs` in `verify-dispatch.ts`; this doc previously and incorrectly described candidates as coming from listing the `aeg-root/iterations/` directory — corrected, forge-sole-state task 1) whose forge-derived task list touches P and currently has zero open task Issues (an `iteration:<slug>` label scan, one batched call per candidate) — the same criterion Section 15b's L1 check uses, applied per-project rather than per-iteration.
...
## Section 15e: Coherence Seam — No Disk State (`check-no-disk-state`, forge-sole-state task 1)

The doctrine underlying every seam above, made a standing invariant rather than a design intent: **GitHub is the sole state machine. No file on disk represents work state** — not an iteration's topology, not its task list, not its dispatch/in-flight/done status, not token/cost accounting. The only durable committed documents are doctrine and reference (specs, skills, role docs, contracts, decision logs) — never a record of *what is currently happening or happened*.

`aeg-root/iterations/completed/**` (topology `.md` files) and every live per-iteration topology `.md` were deleted by forge-sole-state task 1 wherever a forge equivalent exists — proven via `@atta/aeg-forge-state`'s `deriveIterationFromForge`/`parseRationaleDeps`, which already reconstruct a task's id/issue/projects/`dependsOn`/`conflictsWith` from its Issue's title, labels, and "Dependency rationale" body section, and via `listActiveIterationSlugs`/`listArchivedIterationSlugs`, which enumerate iterations from open/closed Milestones instead of a directory listing.

**Two findings this task could not fully resolve, kept and escalated rather than silently worked around:**

1. **A small, closed, non-growing set of legacy iterations predates the Milestone convention (D-110/`aeg-forge-state-v1`) entirely** — no Milestone exists for them, open or closed, so there is no forge fact to derive their archival status from. `GRANDFATHERED_LEGACY_PATHS` (`packages/aeg-core/src/no-disk-state-gate.ts`) names them explicitly; their `completed/*.md` (and, where applicable, `*.tokens.md`) files remain the only record and are excluded from the guard by name, never by a growable rule. Backfilling Milestones for these iterations (mirroring the D-078 rationale backfill `vada-production-v1` already went through, #486) would let them be deleted too — a follow-up task, not this one.
2. **The per-iteration token/cost ledger (`iterations/<name>.tokens.md`, D-071, §13) is not fully forge-derivable under the current design.** Every role's token report already lands somewhere forge-native (PR body, PR/verdict comment) **except two**: the Archivist's own `<task-id>: archive` row (it has no PR of its own to report through) and the Planner's iteration-wide `planning` row (no reliable way to attribute a plan PR to one task from the forge alone, confirmed live building the Studio's own live-read path — `iterations/README.md` §12). This task did **not** delete any `.tokens.md` file or rewrite the role docs (`archivist.md`, `planner.md`, `reviewer.md`, `security.md`, `team-leader.md`) that instruct writing to it, because doing so would silently destroy the only durable record of those two data points. `check-no-disk-state` blocks any *new* `.tokens.md` from being added but leaves the existing ones and the roles' writing obligation untouched. Resolving this for real needs a Type-1 decision: either give the Planner/Archivist a forge-native home for those two row types (e.g. an Issue comment, aggregated the same way the Studio already aggregates PR-body/PR-comment token reports), or accept the gap and retire the ledger anyway. Principal call, not this task's to make unilaterally.

### The guard: `packages/aeg-core/bin/check-no-disk-state.ts`

Sibling to every other seam in this section: a thin CLI shim (`bin/`) over a pure evaluator (`checkNoDiskState`, `src/no-disk-state-gate.ts`). No forge calls, no git history — reads `git ls-files` and classifies each tracked path into one of three outcomes: fine, LIVE violation (any `aeg-root/iterations/*.md` other than `README.md`), or NEW-LEGACY violation (an `aeg-root/iterations/completed/*.md` or `*.tokens.md` path not already in `GRANDFATHERED_LEGACY_PATHS`). Wired as a step of the AEG gate suite job (`.github/workflows/forge-lifecycle.yml`), blocking, ring 1 (`enforcement.md`). Deliberately does **not** attempt a static scan for `readFileSync`/`existsSync` calls resolving under `aeg-root/iterations/` (the brief's optional recommendation) — legitimate forge-primary-with-graceful-file-fallback readers still exist by design (`verify-coherence.ts`'s `deriveOrFallback`, `apps/aeg/web/studio` and `apps/vinaya/web`'s `aeg-fs/read-root.ts` legacy-merge path) and a blanket regex scan would misclassify them; the file-existence guard is the enforcement surface, not a code-shape scan.
