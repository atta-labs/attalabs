...
- **Tier-appropriate documentation** — the `verify-docs` script checks the PR's tier has the corresponding artifact changes; fails CI if missing. **Real**, not a stub. The blocking gate now runs as the `doc-coverage` check of `.github/workflows/vinaya-checks.yml`, which executes the installed CLI's whole check registry in one job. The gate also runs locally (this repo: `bun run verify-docs --pr`). (In observe mode this runs report-only.)
