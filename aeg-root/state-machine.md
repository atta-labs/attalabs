...
The conversational role set is: **Principal, Planner / Brief Author, Developer, Reviewer** (plus the non-conversational Archivist). This four-role model was established (superseding the original three-role to add the Reviewer). The **Planner / Brief Author has three modes** — Strategist, Planner, Brief Author (`roles/brief-author.md`, `roles/planner.md`) — but they are modes of one role, not new roles; the role count is unchanged. Security is a specialization of Reviewer, not a separate role. **Verification (`roles/developer.md` § Verification) is a *phase*, not a fifth role:** it is jointly satisfied by the Developer-agent (the `[agent]` half of the brief's Test Plan) and the Principal (the `[principal]` half), so the role count remains unchanged (June 2026). The per-task **Archivist** named above closes out a single task at merge; a distinct non-conversational role, the **Iteration Archivist** (`roles/iteration-archivist.md`), closes out a whole iteration at Phase 13 — neither is conversational, so both sit outside the conversational role set without changing its count.
...
5. PM docs (iteration files) plus per-project pinned state/lessons Issues — non-derivable operational facts / plan topology (active status derived from forge)
...
**Numbering is per-log, not globally unique.** Each log carries its own `D-###` sequence; numbers collide across logs deliberately. The legacy Vāda log (`apps/vada-ai/specs/vada-decisions.md`) runs its own sequence, as did Cetana's before that product was retired and deleted. There is a Vāda and a global and they are different decisions. **Always disambiguate by naming the log** (e.g. "global", "vada-decisions"). The global log has apparent gaps because some early v3 decisions were filed in project logs. Within any single log, numbers are sequential and append-only; the Archivist validates within-log sequencing (Section 12).
...
- **The per-iteration token/cost ledger** (`aeg-root/iterations/<name>.tokens.md`) — every role reports its tokens at the end of its turn (PR body, verdict comment, or planning report); the per-task Archivist is the sole writer, appending one row per role-turn at close-out; re-entry appends another row; the iteration total is `sum(rows)`, derived at read time, never stored. Same philosophy as derived task status: don't store the aggregate, sum the immutable entries. See `iterations/README.md` §12.
...
So `vinaya/iteration:state-machine-v1` reads as: the Vinaya product, the `iteration` axis, the `state-machine-v1` value. The namespace is what lets every Vinaya label be filtered as one group in a repo it shares with an adopter's own labels — which is the whole reason a product namespace exists. In a `gh` command or a script the name needs quoting (`"vinaya/tier:1"`); in a search URL the `/` encodes as `%2F`.

GitHub caps a label name at **50 characters**, and `vinaya/iteration:` spends 17 of them before the slug starts. `iterationSlugLengthError()` enforces the remainder where an iteration label is first applied (`open-issue.ts`), so an over-long slug fails with a readable message instead of an opaque `gh` 422.
...
### What is explicitly out of scope for **One-time staleness audit** of existing skills/specs to seed the initial bindings — backlog T4.
...
The seam between **iteration topology files** (the plan — `aeg-root/iterations/*.md`) and **the forge** (GitHub Issue state / PR merge events). makes this seam deterministically verifiable: a stateless oracle that detects drift without LLM calls, runnable in CI.
...
| T2 | `orphan-task` | Every open Issue labeled `vinaya/iteration:X` appears in X's topology. **Blocks CI only for a plan PR** (its diff touches that iteration's topology file) as of see "T2 branch scoping" below |
| T3 | `tbd-in-active-iteration` | No `#TBD` rows in an active iteration |
...
- **Studio's actual `/docs` nav rendering** — validated by re-reading the same manifest, not modified. `aeg-studio-cleanup` is where Studio's `load-aeg-docs.ts` is updated to filter by this manifest.
...
### Forcing mechanisms (aeg-governance-hardening task 25, #365) mechanized both `verify-dispatch` and `verify-task`, but neither was *forced* — an agent could simply not invoke them. Task 25 closes that gap for both:
...
### What is explicitly out of scope for **Automatic dispatch** — `verify-dispatch` reports readiness; it does not open worktrees, branches, or PRs. A human or automation layer still decides when to dispatch.
