...
**Only one open plan PR per tranche, at a time — mechanically enforced (task 19), for the case where a plan PR exists at all.** Two concurrent plan PRs for the same tranche, each cut from `origin/main` before the other merged, is the race that produced two competing plan PRs for the same tranche. `node_modules/@attalabs/aeg-core/bin/open-pr.ts`'s single-plan-PR guard (`checkSinglePlanPr`) still refuses outright to open or edit a plan PR whose diff touches a tranche's topology file while another OPEN PR's diff already touches it — relevant now mainly to the historical `completed/*.md` files, since new tranches no longer have a live topology file to race on.
