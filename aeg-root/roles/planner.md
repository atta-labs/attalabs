...
5. **The relevant locked decisions are known.** You've read the decision log entries (`docs/decisions-legacy.md`, `apps/*/specs/*-decisions.md`) that bear on this work, so you don't plan a task that violates a `Lock: YES` or re-litigates a settled call. If you can't find the decision history, STOP.
