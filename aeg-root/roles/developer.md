...
| `Doc-waiver:`    | Optional. `Doc-waiver: <pointer> — <reason>` — per-binding, per-PR escape for an in-repo binding the author intentionally did not update. `<pointer>` must exactly match the binding pointer. Separator is flexible — em-dash `—`, en-dash `–`, or a plain ASCII hyphen `-` (with surrounding whitespace) are all accepted. Logged for audit; the Reviewer judges whether the waiver is justified. **Body field, not a label.** (state-machine.md Section 15) |
