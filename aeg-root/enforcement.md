...
| Starting the Dig (before authoring a brief) / starting Step 0 (before executing one) | **`verify-dispatch` CLI** (`packages/aeg-core/bin/verify-dispatch.ts` — mandated by `roles/developer.md` and `skills/brief-authoring/SKILL.md`; not yet hook-automated) | Every dispatch precondition is re-checked live against a freshly-fetched `origin/main` and the forge: the task's row and Issue exist; the Issue passes the D-078 rationale gate; every `depends-on` PR is merged and no `conflicts-with` PR is open/in-flight; the immediately-prior task's three-predicate archival bar holds; every named project's prior iteration is archived. A `NOT READY` result is a stop condition (D-081). Every `gh` call this script makes carries an explicit `-R <owner>/<repo>` (task 23, #360) — from a linked `git worktree add` checkout, an untargeted `gh issue list`/`gh issue view`/`gh pr list` silently scopes to the wrong repo (or returns nothing), which previously produced false prior-iteration-archival blockers. |
...
| Opening a task PR (final self-check before creation) | **`verify-task` CLI** (`packages/aeg-core/bin/verify-task.ts` — mandated by `roles/developer.md`; not yet hook-automated) | Typecheck, lint, tests, build, `verify-docs --pr`, and the premise coverage/recheck pair all pass, as one summary, against the PR's actual diff, before `open-pr.ts` is invoked (D-081). |
...
- **(b) Manual CLIs lacking a forcing mechanism** — a tool that mechanizes a check (`verify-dispatch`, `verify-task`) but nothing yet *requires* an agent to run it; an agent can simply skip invoking it. Out of this task's scope — task 25's (#365).
...
| `verify-dispatch` (pre-work readiness) | — (CLI exists; not hook-forced) | — | — | Mechanized, **not forced** | **(b) — gap, owned by task 25 (#365)** |
| `verify-task` (pre-PR exit composite) | — (CLI exists; not hook-forced) | — | — | Mechanized, **not forced** | **(b) — gap, owned by task 25 (#365)** |

This table describes **installed mechanisms only** (the page's own change-discipline rule, below). The two family-(b) rows are named here because they are known, already-mechanized gaps — not because task 25 has shipped; when it lands, this table's rows update in that task's own change set, not before.
