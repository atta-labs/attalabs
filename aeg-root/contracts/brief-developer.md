...
> **SUPERSEDED by D-120 (2026-07-13).** This precondition (the per-task archival / row-adjacency gate, D-052 item 1) is no longer a hard-STOP for the Brief Author or the Developer — D-077's automated post-merge provenance posting made the drift signal it protected moot. See `packages/governance/decisions.md` D-120. The section below is preserved as historical record — do NOT enforce it.
...
- **Row-existence precondition (hard STOP before step 0, D-075).** Before executing step 0, `git fetch origin main`, then confirm this task's row exists at all in its iteration topology file (`aeg-root/iterations/<name>.md`) — read from the freshly-fetched `origin/main`, never a stale local checkout or memory. This is distinct from and prior to the Issue-existence precondition below: a missing row means the plan PR that adds this task has not merged yet. STOP: *"Task <id> is not present in `aeg-root/iterations/<name>.md` on a freshly-fetched `origin/main` — the plan PR that adds it has not merged. Not dispatchable until it does."* This gate is enforced in `aeg-root/roles/developer.md` (entry gate, item 7).
