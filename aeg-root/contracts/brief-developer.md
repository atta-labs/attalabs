...
- **A brief with a real §4 code surface must carry a `Premise:` block pinning at least one checkable fact inside that surface (D-080).** See `skills/brief-authoring/SKILL.md` § Premise pins for the grammar and authoring rules. A Tier 0 brief with zero code/runtime surface has nothing to pin.
