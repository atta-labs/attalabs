...
- ~~**Prior-archival precondition satisfied.**~~ **SUPERSEDED by D-120 (2026-07-13).** This is no longer a Developer producer obligation — D-052 item 1 (the per-task archival / row-adjacency precondition formerly defined in `aeg-root/contracts/brief-developer.md`) is removed as a hard-STOP; D-077's automated post-merge provenance posting made the drift signal it protected moot. See `packages/governance/decisions.md` D-120.
