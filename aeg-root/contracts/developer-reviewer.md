...
| **`aeg-root/doc-owners` coverage (C5) satisfied** — every binding fired by the diff is bound (in-diff doc), URL-acked (`Doc-ack: <pointer> — <note>`), or explicitly waived (`Doc-waiver: <pointer> — <reason>`). | Coverage check — *mechanical* via CI; the Reviewer reads it as already-true and moves on. | Coverage presence is mechanical (D-062 — `verify-docs` C5 fails CI if missing). The Reviewer's job shrinks to **judging correctness of the covered doc**: a passing C5 plus an incorrect / no-op / misleading doc update is still a BLOCKER. A `Doc-waiver:` line is the author's explicit deferral; the Reviewer judges whether the reason is real or papering over an obligation that belongs in this PR — a waiver-of-convenience is a BLOCKER. The seam is dormant when `aeg-root/doc-owners` is absent or no binding matches; in that case there is nothing for either side to do. |
...
- **`aeg-root/doc-owners` coverage satisfied (D-062).** For every binding fired by the diff: update the bound doc in this PR; or, for URL bindings, add `Doc-ack: <pointer> — <note>`; or add `Doc-waiver: <pointer> — <reason>` with a real reason. `verify-docs` C5 enforces this mechanically — if it fails CI, do not request review.
