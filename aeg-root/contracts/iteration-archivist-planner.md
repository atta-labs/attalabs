...
This contract formalizes the close-out outputs the Iteration Archivist must produce, and the check the Planner must perform on them. The check is not a checklist item the Planner reviews — it is a forge fact: is the previous iteration's Milestone **closed** (`findMilestoneForSlug`)? If no, the gate fails. (A small, closed, pre-`aeg-forge-state-v1` set of iterations has no Milestone at all — for those the legacy signal is unchanged: does `aeg-root/iterations/completed/<name>.md` exist? See `iterations/README.md` §11.)
...
1. The **closed Milestone** — the physical signal (legacy pre-Milestone iterations: the archived iteration file at `aeg-root/iterations/completed/<name>.md`).
...
|---|---|---|
| **Closed Milestone** (legacy: archived iteration file at `aeg-root/iterations/completed/<name>.md`) | Readiness gate item 8 | The Planner MUST confirm the previous iteration's Milestone is closed before planning any new iteration on the same product. An open Milestone (or, for a legacy iteration, an unmoved file) means the Iteration Archivist has not run — planning is blocked. |
...
- Close the iteration's Milestone — this is the **physical signal** the Planner's gate checks (legacy pre-Milestone iterations: move the iteration file to `completed/` instead). A close-out that does everything else but fails this step is an incomplete close-out that correctly blocks planning.
...
- Run readiness gate item 8 before planning any iteration that includes a product: confirm the previous iteration's Milestone is closed (`findMilestoneForSlug`) for each product in scope — or, for a legacy pre-Milestone iteration, that `aeg-root/iterations/completed/<name>.md` exists.
- If any prior iteration on an in-scope product has an open Milestone (or, for a legacy iteration, its file is still outside `completed/`), STOP: *"The previous iteration `<name>` on `<product>` has not been archived — the Iteration Archivist has not run. Dispatch the Iteration Archivist for `<name>` before planning proceeds."*
- Do not improvise around a missing close-out. "The Iteration Archivist probably ran" is not a passed gate. The forge fact (or, for a legacy iteration, the file check) is the gate. If it isn't there, stop.
...
1. **Planner refactor-and-plan first.** The Planner plans the destination, relabels the moved Issues (`iteration:<src>` → `iteration:<dest>` + provenance comment), and annotates the move (`Moved out → <dest>`) as a comment on the source Milestone's tracking Issue, or, for a legacy pre-Milestone iteration, on its still-open source topology file. Moving tasks is the Planner's power; the Archivist neither moves them nor decides the destination.
