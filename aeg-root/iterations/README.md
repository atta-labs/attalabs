...
**Ratifies via:** (global decisions.md)
...
1. **The conflict rule binds across iterations (§5).** Two concurrent iterations are safe to run fully in parallel **only if their tasks do not share a collision domain.** If iteration A and iteration B both touch `@atta/ui`, their colliding tasks must serialize across the iteration boundary exactly as if they were siblings — the gate doesn't care which file a PR's task belongs to. **The cleanest concurrency is between disjoint iterations** (different projects, no shared package), where nothing can collide by construction. Example: `herald-onto-engine` (herald + engine) and a `vinaya-pages` iteration (the `apps/vinaya/web` app) share no collision domain, so they run fully parallel with zero cross-checks needed.
