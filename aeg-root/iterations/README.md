...
**Cutover status (`aeg-forge-state-v1` task 7, #431) — partially complete, not fully complete.** The birth rule above ("gates read files until the migration flips the config in one deliberate act") has now flipped for every active iteration **except one, tracked exception**: `vada-production-v1`. Its thin file (`aeg-root/iterations/vada-production-v1.md` + `.tokens.md`) was deliberately kept, not deleted, because 9 of its Issues (#183 #184 #185 #186 #187 #188 #240 #241 #244) predate the D-078 "Dependency rationale" grammar and carry no forge-parseable dependency data — deleting the file would silently blank `dependsOn` for those tasks in every gate and in Studio, reintroducing exactly the "silently permissive D1 gate" failure the original stop condition existed to prevent. Backfilling those 9 Issues with real, per-task rationale (not a mechanical append) is its own follow-up task, not yet dispatched. Every other active iteration's topology file (`herald-hardening-v1.md`, `vinaya-cli-v1.md`, `vinaya-studio-v1.md`) was deleted by task 7 once its forge-native replacement was proven live — `dependsOn`/`conflictsWith` for those is now genuinely forge-derived, no file fallback. `completed/*.md` files are never deleted, by design (§11) — the birth rule never applied to them. This iteration's own file (`aeg-forge-state-v1.md`) stays until this PR merges, then follows the normal close-out archival flow (§11), not this task.
