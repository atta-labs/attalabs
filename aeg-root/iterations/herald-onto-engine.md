...
> **This is the first iteration run as a deliberate planning prototype** — every task carries a `Planner's rationale` block (candidate model addition F1) and a suggested agent-class (F4). The sizing reflects a deep code dig (F2) that overturned a backlog claim (F5): multi-vendor structured output is a shared-engine change with every engine consumer (today, Vāda) in the blast radius, not "just UI."
...
- **THE ENGINE IS SHARED — EVERY CONSUMER IS IN THE BLAST RADIUS (today: Vāda).** `llm.ts` is a shared file every engine consumer runs on. This is not "a Herald favor" — it is the shared substrate maturing; the next project that needs structured output on Google gets it for free. But it means the change MUST re-verify every current consumer's behavior (today Vāda's deliberations still run), and may require editing a consumer's call sites IF the structured-output contract changes (vs. purely additive new vendor branches). Vāda is listed as a project for that reason — the Reviewer must check Vāda, not just Herald. **Aim for additive** (new branches that only fire for non-Anthropic + outputSchema) so existing consumers hit their untouched path → re-verify only, no edit.
- **Three projects (engine, vada, herald):** the change is in the shared engine (engine); Vāda consumes the changed file (vada, re-verified as the current consumer); Herald is the consumer that drives the requirement (herald).
