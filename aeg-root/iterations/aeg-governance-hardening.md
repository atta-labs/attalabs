...
- **Fix punch-list (tasks 6…n) — spawned by task 3.** The staleness audit in task 3 (#218) emits one new Issue per contradiction it finds between a newly-bound doc and `D-001 … D-070`. Those Issues are this iteration's fix punch-list. Numbering is sequential from the next free integer at dispatch time; the set is cut from real audit findings, not pre-enumerated here.
