...
- **Task 30 (#393) — the templates task, from the same live incident that produced tasks 21/25's Test Plan fix.** PR #392's pasted reference-brief duplicated its own Test Plan template inside the PR body — hand-fixed live (2026-07-04) by wrapping the reference copy in a collapsed `<details>` block; this task makes that the standing convention, generalizes anchored/delimited fields (Tier, Project, Closes #N, Premise, Test Plan) beyond just Test Plan, and replaces prose-described artifact shapes (Issue rationale, brief, PR report) with literal `aeg-root/templates/*.md` files agents copy and fill in. Root cause named explicitly in the rationale: every malformed-artifact incident this iteration (task 21, #363, #377, #392) traces to gates reverse-engineering structure from freehand prose instead of reading a fixed, unambiguous location.
- **Task 29 (#380) — registered by the vinaya plan PR (Planner act, 2026-07-04).** Implements D-097 (R-16 of the Vinaya review program; supersedes D-080 upon ratification): waivers become a `waiver:docs` PR label verified by the ACTOR of the labeling timeline event against a principal allowlist; the agent-emittable `Doc-waiver:` body/trailer grammar is removed from CI-accepted inputs; ring 0 downgrades to warn-with-declared-intent; the tool-layer forge gate denies waiver-label mutations in agent sessions (shared-credential hole). `Conflicts-with: 25` — task 25 is in flight on the same `.husky/pre-push` + waiver-detection path; 29 lands on top of whatever 25 ships. Placed here rather than in a Vinaya iteration because it patches the live gate engine, and the mechanism must be proven in this repo before Vinaya productizes it (vinaya-cli-v1 #387 depends on it). Backlog note for a later iteration: a separate machine identity for agents is the clean long-term fix for the shared-credential caveat.
