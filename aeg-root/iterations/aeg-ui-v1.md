...
|---|------|-------|-----------|------------|----------------|
| 1 | `@atta/aeg-core` — AEG-artifact parser + `deriveIteration` (pure) | #TBD | aeg-core | — | — |
| 2 | Scaffold `apps/aeg/web/studio` + top-bar/sidebar shell | #TBD | aeg | — | — |
| 3 | Local GitHub read adapter → forge facts for `deriveIteration` | #TBD | aeg, aeg-core | 1 | — |
| 4 | Projects + iterations pages (sidebar nav, topology table, active/archived) | #TBD | aeg | 1, 2 | 5, 6 |
| 5 | Kanban + task detail (columns by derived status; brief from PR body) | #TBD | aeg | 1, 2, 3 | 4, 6 |
| 6 | Task-dependency-graph view (`@atta/ui/engine-flow`) | #TBD | aeg | 1, 2 | 4, 5 |
| 7 | Shared docs renderer in `@atta/aeg-core` + Studio docs section | #TBD | aeg-core, aeg | 1, 2 | — |
| 8 | AEG icon — find/design | #TBD | aeg | — | — |

**Wave structure (max concurrency, from the edges):**
- **Wave 1 (parallel):** 1 (`aeg-core` parser), 2 (shell scaffold), 8 (icon) — three independent foundations, no shared surface. (2 can stub data until 1 lands; 8 is pure asset work.)
- **Wave 2:** 3 (GitHub read) — needs 1's types.
- **Wave 3 (serialized within the app surface):** 4, 5, 6 all build pages inside `apps/aeg/web/studio` and share the app's routing/layout surface, so they **conflict with each other** (same collision domain: the Studio app shell wiring). Run them one at a time: **4 → 6 → 5** (4 establishes the project/iteration nav the others hang off; 6 is read-only-ish graph; 5 is the richest, needs 3's live status + PR-body read, so last). 7 (docs) touches `aeg-core` + a separate docs route, so it can run **parallel to one** of 4/5/6 if desired, but to stay safe it's simplest to slot it after 4.
...
### Task 1 — `@atta/aeg-core`: AEG-artifact parser + `deriveIteration` (pure)
...
### Task 2 — Scaffold `apps/aeg/web/studio` + top-bar/sidebar shell
...
- **Traps to avoid:** **do not edit `@atta/ui`** — reuse `TopBar` via its existing props (`isSignedIn`/`accountMenu` exist per D-036; Studio is no-auth so pass the no-auth shape). Follow the per-library UI resolution rules if they apply, but Studio is internal/infra — confirm whether it uses the CMS-library chrome pattern (like Herald, D-035) or plain `@atta/ui` (simpler; likely correct for an infra tool). Copy the Next.js worktree-root resolution fix the other apps needed (`next.config.ts`).
...
### Task 3 — Local GitHub read adapter → forge facts for `deriveIteration`
...
### Task 4 — Projects + iterations pages (sidebar nav, topology table, active/archived)
...
### Task 5 — Kanban + task detail (columns by derived status; brief from PR body)
...
### Task 6 — Task-dependency-graph view (`@atta/ui/engine-flow`)
...
- **Issues not yet cut** — task→Issue map is `#TBD` until they're created; then this table goes live and the file is topology-only.
- **Dispatch order:** wave 1 = **1, 2, 8** in parallel (parser, shell, icon). Then 3. Then the app-surface wave **4 → 6 → 5** serialized (shared shell), with 7 slottable after 4.
