...
- **The living state** → `aeg-project/`: one at the repo root (for monorepo-level work) and one per unit (`apps/<x>/aeg-project/`, `packages/<y>/aeg-project/`). Holds only `decisions.md` — never the model. (`now.md` is retired —; active/blocked/next is derived from the forge. Completed-work history, lessons, per-project operational state, and ratification items are forge-native —.)
