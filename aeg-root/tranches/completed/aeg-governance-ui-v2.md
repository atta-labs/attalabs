...
---
# Tranche: aeg-governance-ui-v2 — June–July 2026
Lifecycle: complete

Goal: Complete the AEG model by writing all missing role-seam contracts and adding the
Planner readiness gate that enforces tranche close-out before new planning begins
(task 1 — blocks both other tranches). Then refactor AEG Studio with the science-route
UI pattern, add a cross-product tranches view, and complete the token ledger Studio display.
...
| 3 | Tranches cross-product view — new `/tranches` route showing all active + archived tranches across all projects in one view; reads `aeg-root/tranches/` + `aeg-root/tranches/completed/`; derives status from forge | #TBD | aeg, aeg-core | 2 | — |
| 4 | Token ledger Studio display (#110-view) — per-task and per-tranche totals on the tranche view; consumes `parseLedger` + `sumLedger` from `@atta/aeg-core` (already built, D-048) | #TBD | aeg, aeg-core | 2 | — |
...
that checks for completed tranche close-out on every product in scope before planning begins.
...
- `contracts/archivist-tranche-archivist.md` — what per-task close-outs must have completed
  before the Tranche Archivist can declare a tranche fully archivable
- `contracts/tranche-archivist-planner.md` — what the Tranche Archivist must have
  produced, what the Planner must verify before planning a new tranche on that product
...
- Item 8 in the readiness gate: "For every product in scope, confirm the previous tranche
  is in `aeg-root/tranches/completed/`. If any prior tranche on that product exists in
  `aeg-root/tranches/` but not `completed/`, the Tranche Archivist has not run — STOP
  and instruct the Principal to dispatch the Tranche Archivist for that tranche before
  planning proceeds."
...
any tranche files or state docs — governance docs only.
...
structure verbatim; adapt nav items to AEG's content (Projects, Tranches, Docs, Graph);
...
### Task 3 — Tranches cross-product view
**Boundary:** New `/tranches` route in AEG Studio. Reads `aeg-root/tranches/` (active)
and `aeg-root/tranches/completed/` (archived). Displays all tranches across all products
in one view — not per-product. Two sections: Active (files in `aeg-root/tranches/` without
`completed/` path) and Archived (files in `aeg-root/tranches/completed/`). Derives status
from the tranche file's `Lifecycle:` marker and the task topology. Uses `@atta/aeg-core`'s
`parseTranche` for file parsing. Sidebar nav: `/tranches` entry under a new "Tranches"
group.
**Sizing:** New route. One PR. Single verification story (route renders active + archived
tranches). Bounded (new route file + sidebar entry + aeg-core parser reuse). Passes all.
...
that is Phase 2 AEG Studio work (the forge-fact cache and `deriveTranche`). This task
reads the tranche FILES only and displays their topology + lifecycle marker. Status is
...
**Stop-and-escalate:** If `parseTranche` in `@atta/aeg-core` needs structural changes to
...
**Boundary:** Display per-task and per-tranche token/cost totals on the tranche view
in AEG Studio. Consumes `parseLedger` and `sumLedger` from `@atta/aeg-core` (both built
and tested in D-048 / task 9 model half). Reads `aeg-root/tranches/<name>.tokens.md`
sibling files. Shows totals inline on the tranche detail page. This is the Studio view
half of the original task 9 (#110) that was left unbuilt.
**Sizing:** Small. One PR. Single verification story (tranche view shows ledger totals).
Bounded (tranche detail page component + ledger file reads). Passes all four tests.
**Project(s) + blast radius:** aeg (Studio display), aeg-core (consumed unchanged).
**Dependency rationale:** Depends on task 2 — the tranche detail page is part of the
...
- GitHub App auth + encrypted token store — deferred to next AEG tranche.
- Webhook-fed forge-fact cache + `deriveTranche` integration — deferred.
...
- Task 1 here has no dependencies and must merge BEFORE herald-agents-v2/1 dispatches
  ...
  other tranches' Planner readiness gates.

## Shipped

- Token ledger view shipped on tranche detail page (parseLedger + sumLedger, D-048).
