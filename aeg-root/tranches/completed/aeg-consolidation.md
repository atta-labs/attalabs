# Tranche: aeg-consolidation — July 2026
Lifecycle: complete
...
- **`aeg-governance-hardening`** depends on this tranche: its task 1 (#251) depends on this tranche's task 2 (#264); its task 2 (#252) and task 3 (#218) each depend on this tranche's task 1 (#263). Dispatch gates are forge-answerable regardless of which tranche file lists the task — do not dispatch those `aeg-governance-hardening` tasks until the referenced Issue here is merged.
- **`aeg-studio-cleanup`** (not yet planned) will depend on this tranche's task 4 (#265) — it defines the "surfaced doc" manifest that tranche's docs-curation task consumes.
- Superseded, absorbed the unbuilt tail of `aeg-coherence-v1` (D-070 movement, 2026-07-01): task 3 (#220) is that tranche's former task 5, re-scoped after a deep-dig into the actual code found the two real CI≠local bugs its old exit criteria depended on. See `aeg-root/tranches/aeg-coherence-v1.md` for the per-task move annotations.
