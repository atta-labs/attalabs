...
- **A `ColorSchemeToggle` row lives INSIDE the sheet too**, not only in the collapsed bar next to the hamburger trigger — the collapsed-bar one becomes unreachable the moment the sheet opens (a full-screen overlay), which otherwise strands theme-switching behind a close-then-reopen round trip (#816).
