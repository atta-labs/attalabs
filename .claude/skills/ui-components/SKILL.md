...
**`chromeClassName?: string`** is merged onto the `topbar`-variant `ChromeFrame`, after its own default classes — a consumer overrides e.g. the background opacity for its own effect (Vinaya's glass topbar, `bg-background/35 backdrop-blur-md`) without touching the shared component. Unset by every other consumer, so it's a no-op everywhere it isn't explicitly passed. Threaded through both `TopBarWithAuth` and `TopBarNoAuth`. See "The nav frame is library-resolved via `ChromeFrame`" below, and `.claude/skills/ui-library-system/SKILL.md`'s `data-bare` note, for the companion mechanism this pairs with — an ancestor attribute that gates the frame's *border/background*, orthogonal to this className override.
