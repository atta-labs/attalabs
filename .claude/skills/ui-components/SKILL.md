...
**`chromeClassName?: string`** is merged onto the `topbar`-variant `ChromeFrame`, after its own default classes — a consumer overrides e.g. the background opacity for its own effect (Vinaya's glass topbar, `bg-background/35 backdrop-blur-md`) without touching the shared component. Unset by every other consumer, so it's a no-op everywhere it isn't explicitly passed. Threaded through both `TopBarWithAuth` and `TopBarNoAuth`. A consumer that needs a *state-dependent* frame keys the state off its own wrapper and carries the rule in this same string, never in `ChromeFrame`: Vinaya's landing cold-open sets `data-bare="true"` on its own `TopBarChromeHost` wrapper and passes `[[data-bare=true]_&]:bg-transparent` here, so the shared frame has no knowledge of the attribute at all. See "The nav frame is library-resolved via `ChromeFrame`" below.
...
- **Desktop (≥ lg):** a flat item renders exactly as before, a plain `NextLink`. A group renders its own `NavigationMenu` instance (library-resolved via `useComponents()`, basic fallback — same precedent as `ChromeFrame`/`Button` above) sitting inline among the flat links: a trigger styled to match `NextLink variant='nav'` metrics (`text-xs`, `text-muted-foreground` / `hover:text-primary`, active `text-primary font-medium` — active means *any* item inside the group is active), and a panel listing each item as icon + label + optional muted one-line `description`. The trigger is a real button (Radix `NavigationMenuTrigger`) — it never nests inside a `NextLink`/`<a>`, and a group never carries its own `href`; only its items navigate. The desktop/mobile split is `lg` (1024px), not `md` — matching Vinaya's own `/start` sidebar breakpoint (#816); see "The nav frame is library-resolved via `ChromeFrame`" below for why this is a single shared value every `TopBar` consumer moves together on, not a per-app override.
- **Mobile (< lg, inside the hamburger sheet):** a group renders a non-navigating header row (icon + label, muted, `h-12` like its siblings) followed by its items as the same `SheetClose`+`NextLink` rows flat links use, just indented — there is no dropdown inside the sheet. The sheet body (`<nav>`) is `flex-1 min-h-0 overflow-y-auto` — a real scroll region, not the fixed-header-plus-unbounded-content shape it used to be — and every row carries `shrink-0`: without it, a flex column's default `flex-shrink: 1` COMPRESSES fixed-height children to fit the available space instead of actually overflowing, which silently defeated `overflow-y-auto` (rows visually shrank below their own `h-12` rather than the sheet scrolling) until this was caught live and verified via CDP (`nav.scrollHeight > nav.clientHeight`, then simulating a real scroll to confirm the last row becomes reachable — not just that the CSS property was present) (#816). The installed `SheetContent`'s own `gap-4` (from `installed/sheet.tsx`, its default child-spacing) added a stray gap between the header row and the nav below it; overridden to `gap-0` at the call site.
- **The row group is vertically centered inside `<nav>` via `my-auto` on an INNER wrapper, never `justify-center` on the scroll container itself.** `justify-center` on `<nav>` centers overflow symmetrically on both sides of the scrollable range — when content is taller than the sheet, that clips the FIRST row above `scrollTop: 0` with no way to scroll back to it (measured via CDP: `firstRow.top` sat at `-66px` even at minimum scroll, a real regression this caught before shipping). `margin: auto` on the inner wrapper instead centers it ONLY while there's surplus space in `<nav>`; the instant content exceeds the available height the auto margins collapse to `0` and the list simply starts at the top, fully scrollable either way — the standard fix for "center when it fits, scroll when it doesn't." The last row's `border-b` is stripped via `[&>*:last-child]:border-b-0` on that same inner wrapper, positionally targeting whichever row ends up last regardless of auth state (`extraActions` in `TopBarNoAuth`, `accountMenu`/sign-in in `TopBarWithAuth`) rather than hardcoding an assumption about which row that is (#816).
- **The sheet's open state is controlled, watching `(min-width: 1024px)`, and auto-closes on resize.** The hamburger trigger's own `lg:hidden` only hides the BUTTON below `lg` — it says nothing about an ALREADY-OPEN sheet, since Radix's `Dialog`/`Sheet` open state is plain React state, not CSS. Opening the sheet at a narrow width and then widening the browser back past `lg` (without an explicit close) used to leave it stuck open, full-screen, on top of the now-available desktop nav — a real bug found live. `useAutoCloseMobileSheet()` lifts `<Sheet>` to `open`/`onOpenChange` and force-closes it the instant a `matchMedia('(min-width: 1024px)')` listener fires — the `1024` must keep matching `lg`'s own value (Tailwind's default, unchanged here) or the two diverge again (#816).
- **The sheet's own header row (logo … `ColorSchemeToggle` … close button) carries a second `ColorSchemeToggle`**, not only the one in the collapsed bar next to the hamburger trigger — the collapsed-bar one becomes unreachable the moment the sheet opens (a full-screen overlay), which otherwise strands theme-switching behind a close-then-reopen round trip. Lives in the HEADER, left of the close button, not as a row inside the scrollable nav list — a first attempt put it there and was corrected: it reads as a persistent chrome control (paired with the same close-button row every other header carries it next to), not a fourth nav item mixed in among Home/Start/Docs (#816).
...
- **Icons:** `TopBarLink`/`TopBarGroupItem` both carry an optional `icon?: ReactNode`, rendered before the label in the desktop row and the sheet row. When absent, the row's markup is byte-identical to before the icon slot existed — there's no empty wrapper span waiting for an icon that never comes. A consumer wanting the icon to sit INSIDE a decorated `label` (e.g. inside `ElectricLabel`'s own active-state border, rather than stranded outside it) skips this `icon` field entirely and folds the icon into the `label` node's own children instead — Vinaya's site topbar (#816) does this for every flat item and the `Docs` group trigger.
- **Group trigger vertical alignment:** the real fix is `DesktopNavGroup`'s `<NavigationMenuItem className='flex items-center'>` (`packages/ui/topbar/index.tsx`) — the installed `<li>` (`display: list-item`) doesn't flex-center its own child and inherits an ambient line-height taller than the trigger's own `text-xs`, so the trigger sat flush at the `<li>`'s top rather than centered in it; making the `<li>` itself a flex container centers its one child directly. Confirmed by measuring real `getBoundingClientRect()`s via CDP, not just reading source — the trigger's `centerY` now matches the flat `<a>` siblings' to the pixel. `groupTriggerClassName`'s own `leading-none` is a belt-and-braces addition (same fix `Button`'s universal default applies), not sufficient alone (#816).
- **Chevron hover state, and relocating the chevron into a decorated label:** the installed chevron (`ChevronDownIcon`, `installed/navigation-menu.tsx`) only rotates on the panel's own open state, and — being auto-appended by the installed component AFTER whatever `children` a caller passes — can never be relocated INSIDE a decorated label's own active-state border (e.g. `ElectricLabel`'s lightning), since `installed/` is off-limits to edit. `groupTriggerClassName` hides it (`[&>svg:last-child]:hidden` — a direct-child selector, so it only ever matches the installed component's own auto-appended chevron, never a caller's icon nested deeper inside their label) and adds `[&:hover_.lucide-chevron-down]:rotate-180`, which then targets whichever chevron IS visible: a caller's own replacement icon inside their label (Vinaya's Docs trigger renders its own `ChevronDown` there, with `group-data-[state=open]:rotate-180` for the open-state rotation the installed one used to provide), or the installed one for a group whose label supplies no replacement — this is a **contract now, not an incidental default**: every `TopBarLinkGroup` consumer's trigger loses its installed chevron and must supply its own if it wants one visible (#816).
