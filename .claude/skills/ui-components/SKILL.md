...
- **Group trigger vertical alignment:** the trigger is a native Radix `<button>` (`NavigationMenuTrigger`), whose default line-height sits a few px taller than the flat items' `<a>` siblings — `groupTriggerClassName` (`packages/ui/topbar/index.tsx`) carries `leading-none` for exactly this reason, the same fix `Button`'s own universal default applies (#816; see the `leading-none` paragraph above).
- **Chevron hover state:** the trigger's installed chevron (`ChevronDownIcon`, `installed/navigation-menu.tsx`) only rotates on the panel's own open state — `groupTriggerClassName` adds `[&:hover_.lucide-chevron-down]:rotate-180` so it also flips before the panel opens, targeting the chevron by lucide's own auto-generated `lucide-chevron-down` class (never `[&:hover_svg]:...`, which would also catch a caller's own icon folded into the trigger's label per the point above) (#816).
