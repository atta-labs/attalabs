...
**Distribution:** published as `@attalabs/vinaya` (bare `vinaya` is unobtainable — npm's typosquat filter, not a taken name), a single Node-executable ESM bundle inlining the whole `@atta/*` workspace graph (`aeg-core`, `aeg-forge-state`, `aeg-types`, `vinaya-sources`). The `files` allowlist (`dist/`, `templates/`, `aeg-root/`, `studio-standalone/`, `README.md`) is the tarball's actual contents — a file that isn't an `artifacts.ts` op AND isn't in this list never reaches an adopter, whatever else it may be (`specs/vinaya-spec.md`'s "Vinaya's own vs. attalabs' own" classification table is the authoritative three-way split when this is ambiguous).

**`studio-standalone/` — Studio ships runnable outside the workspace.** `cli/scripts/bundle-studio.ts` runs `next build` (`output: 'standalone'`, set in `web/next.config.ts`) against `web` from inside the monorepo — the only place workspace resolution of `@atta/*` and the `@atta/ui` codegen step work — then copies the result into this directory, wired into `prepack` alongside the existing `build`/`bundle-doctrine` steps. `studio.ts`'s `resolveStudioTarget()` now has a real package branch: outside a workspace checkout, it locates `<install-root>/studio-standalone/apps/vinaya/web/server.js` via `packageRoot()` and runs it directly, instead of the old `missing` stub. Two packaging quirks this depends on, both handled in `studio.ts`/`bundle-studio.ts` rather than in `web`: npm/bun's packer unconditionally strips any directory literally named `node_modules`, so the bundle's own `next`/`react`/`sharp` ship as `_node_modules` and get renamed back lazily on first run; and Next's generated `server.js` calls `process.chdir(__dirname)` as its own first line, so `AEG_REPO` is resolved from the caller's cwd *before* spawning and forced into the child's env — the one thing that makes the guest repo's identity survive that chdir.
