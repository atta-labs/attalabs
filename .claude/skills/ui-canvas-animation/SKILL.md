...
├── herald-logo.tsx        — Herald's real brand logo (Sanity `branding-herald`), reused verbatim as `currentColor` SVG inside an AIASphere's clipped children slot — one of the TWO sanctioned SVG marks in that hero (Issue #710's Boundary sanctions both), a real product asset not a decorative illustration (AttaLabs ecosystem hero's Herald node)
