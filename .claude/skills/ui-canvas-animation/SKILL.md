...
├── hero-fabric.tsx        — Local, contained fabric canvas (not fixed inset-0) for a normal in-flow section; moved here from the original combined Vinaya web app (since split into apps/vinaya-portal/web), which needed no import rewriting (it already depended only on this package). Gained one optional `config?: Partial<FabricConfig>` prop on the move, merged over the defaults at mount, so a second consumer can vary the fabric without forking it — Vinaya passes no `config`, so its rendering is unchanged
