---
name: aeg-model
description: The AEG governance model — the four truth domains, the three-ring enforcement architecture, tranche lifecycle, and the role/contract seam. Load when working inside aeg-root/** — today that means this repo's own execution history (`tranches/**`) and the three operational-prose files that stay local (`aeg-manual-flow.md`, `documentation-coherence.md`, `reviewer-prompt.md`); the portable doctrine text itself (`enforcement.md`, `state-machine.md`, `tranche-model.md`, `roles/*.md`, `contracts/*.md`, `templates/*.md`, `process.md`, `coordination.md`, `glossary.md`) no longer lives in this repo — it ships inside the installed `@attalabs/vinaya` package, resolved via `npx --yes @attalabs/vinaya doctrine`. Do NOT load for how the gates ship as a product (apps/vinaya) — see the vinaya-architecture skill for that. The gate implementations themselves (`@attalabs/aeg-core`, `@attalabs/aeg-forge-state`) are published, registry-only dependencies with no local skill of their own.
---
...
**Where this tree physically lives, for this repo specifically.** attalabs carries no local copy of the portable doctrine text — editing `enforcement.md`, `state-machine.md`, `tranche-model.md`, `roles/*.md`, `contracts/*.md`, `templates/*.md`, `process.md`, `coordination.md`, or `glossary.md` happens in the standalone `atta-labs/vinaya` repo (canonical source), never here; this repo resolves the current bundled copy at read time via `npx --yes @attalabs/vinaya doctrine`. This repo's own `aeg-root/` holds only two things the published package deliberately does not bundle: this repo's own execution history (`tranches/**` — see the Tranche lifecycle section below) and three operational-prose files (`aeg-manual-flow.md`, `documentation-coherence.md`, `reviewer-prompt.md`) that are this-repo-specific narration rather than portable doctrine. Editing either of those two local, still-real trees is still editing this rulebook — the point below stands for them.
