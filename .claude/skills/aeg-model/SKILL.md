---
name: aeg-model
description: The AEG governance model — the four truth domains, the three-ring enforcement architecture, tranche lifecycle, and the role/contract seam. Load when working inside aeg-root/** — today that means only this repo's own execution history (`tranches/**`); every doctrine text file (`enforcement.md`, `state-machine.md`, `tranche-model.md`, `roles/*.md`, `contracts/*.md`, `templates/*.md`, `process.md`, `coordination.md`, `glossary.md`, plus the operational-prose files `aeg-manual-flow.md`/`documentation-coherence.md`/`reviewer-prompt.md`) has been deleted from this repo — the portable subset resolves via the installed `@attalabs/vinaya` package (`npx --yes @attalabs/vinaya doctrine`); the three operational-prose files have no local copy and no package resolution — read them from the standalone `atta-labs/vinaya` repo's own `aeg-root/` if needed. Do NOT load for how the gates ship as a product (apps/vinaya) — see the vinaya-architecture skill for that. The gate implementations themselves (`@attalabs/aeg-core`, `@attalabs/aeg-forge-state`) are published, registry-only dependencies with no local skill of their own.
---
...
`aeg-root/` is not documentation *about* a system that lives elsewhere — it **is** the system, expressed as prose. Agentic Execution Governance (AEG) is the discipline layer that governs how a human's intent becomes reviewed, merged, coherent code produced by agents; `aeg-root/` is its normative spec. Every hook, every CI check, every CLI in the published `@attalabs/aeg-core`/`@attalabs/aeg-forge-state` is a *mechanization* of a rule stated here first — never the other way around. Get the model wrong here and every downstream enforcement point silently inherits the mistake — this is the tree `enforcement.md`'s own closing line calls out: *"a change that adds, removes, or weakens a gate must update this page in the same change set."*

**Where this tree physically lives, for this repo specifically.** attalabs carries no local copy of the doctrine text at all. The portable subset — `enforcement.md`, `state-machine.md`, `tranche-model.md`, `roles/*.md`, `contracts/*.md`, `templates/*.md`, `process.md`, `coordination.md`, `glossary.md` — is edited in the standalone `atta-labs/vinaya` repo (canonical source) and resolved here at read time via `npx --yes @attalabs/vinaya doctrine`. Three further files (`aeg-manual-flow.md`, `documentation-coherence.md`, `reviewer-prompt.md`) are this-repo-specific operational prose the published package deliberately never bundled — deleted from attalabs too, since a near-identical copy already lives in `atta-labs/vinaya`'s own `aeg-root/` (that repo is itself AEG-governed and keeps its own hand-adapted copy; there is no live resolution path for these three, only that repo's checkout). This repo's own `aeg-root/` now holds only its own execution history (`tranches/**` — see the Tranche lifecycle section below); editing that tree is still editing this rulebook.
...
`planned → active → complete → archived`. A tranche is `active` the moment one task has an open branch; `complete` only when **every** task's PR is merged (derived, never declared); `archived` when the Tranche Archivist closes the tranche's Milestone — closing the Milestone **is** the lifecycle transition, a forge action, not a repo commit. The Issues stay closed and attached to the closed Milestone as the durable historical record. A pre-cutover tranche that still carries a topology file at `aeg-root/tranches/<name>.md` is the one legacy exception: that file moves to `aeg-root/tranches/completed/` (never deleted) instead.

### The role / contract seam

Eight roles (`roles/*.md`), each self-locating: Planner, Brief Author, Developer, Reviewer, Security, Archivist, Tranche Archivist, Principal — plus the manual-flow narration (`aeg-manual-flow.md`), which is not a `roles/*.md` role. A role validates its **own** preconditions from live forge state before acting and refuses when it isn't its turn — the forge just sends a notification; role self-location is one of the four things AEG adds over raw GitHub (`tranche-model.md` §10). `contracts/*.md` are the typed seams *between* consecutive roles (e.g. `brief-developer.md`, `developer-reviewer.md`) — what one role guarantees to hand the next. A role's frontmatter (`role_id`, `performs`, `refuses_when`) is itself machine-read: `verify-registry.ts`'s G5 check fails CI if a contract cites a `role_id` that isn't defined, or a role's frontmatter is incomplete.
