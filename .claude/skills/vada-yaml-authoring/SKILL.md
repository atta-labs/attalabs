...
**Critical**: the synthesis template must use `{{#each allPreviousOutputs}}[{{this.agentName}}] {{this.content}}{{/each}}` to receive the reviewer responses. The pre- v1 YAML referenced `{{reviewerResponses}}` — the engine never populated that variable, and the synthesizer ran blind in production. The PR #47 migration fixed this; do not reintroduce the broken pattern.
...
`validateAllSpecs()` runs at startup — a malformed YAML crashes the server on start. This is intentional (fail-fast). The 10 validation rules from are enforced by `validateFlow` (called by `loadFlow`).
...
- ❌ Defining team logic in TypeScript — it belongs in YAML (`@vada/teams` was deleted long before).
...
- ❌ Adding `-v1` / `-v2` suffix to a filename or `id` — global + vada-decisions.md keep filenames unversioned. Version history lives in git + decision logs.
...
- vada-decisions.md + (engine vocabulary + signal type cleanup)
