...
- **Decision-log history** (`aeg-project/decisions.md`) — existing entries D-001…D-040 are append-only and MUST NOT be edited for vocabulary (they are historical record). Only ADD D-041. **Do not rewrite history.** If a prior entry references `project-management/`, leave it — it was true when written; let D-041 record the rename.
...
- `grep -rIn 'docs:index\|docs-index' package.json` then run the generator; confirm `docs-index.md` shows `aeg-root/` + `aeg-project/` paths and no `project-management/`.
...
- confirmation `bun docs:index` ran and `docs-index.md` was regenerated (or why it was hand-edited),
- `git diff --stat` against the pre-change head,
