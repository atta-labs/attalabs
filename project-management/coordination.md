...
3. `project-management/roles/{your-role}.md` — Team Leader (incl. Planner & Brief Author modes), Developer, Principal, Reviewer, Security, or Archivist
4. `project-management/iterations/README.md` — the iteration model: tasks-as-Issues, forge-derived status, the thin topology file, conflicts (read when planning or executing)
5. `project-management/state.md` — what is true right now across the ecosystem
6. `project-management/now.md` — what is active, what is next, what is blocked
7. `project-management/iterations/<name>.md` — the current iteration's task topology (the plan); live status is queried from the forge, not read here
...
- `project-management/diagrams/` — process and architecture diagrams (note: being brought in line with the forge-derived-status model; if a diagram shows agents writing status, the prose docs are canonical)

Do not generate strategy, plan an iteration, or author briefs until you have read `state-machine.md` and the spec for any product in scope.
...
| `project-management/iterations/<name>.md` | The current iteration's task topology (edges, grouping). Plan only — no status. | At plan time (Planner) |
...
State changes: a product phase advances, an app ships/scaffolds, auth/DNS config changes, any "what is true right now" fact. The TL updates `state.md` (commit or PR) before the session ends. For Tier 3 work, the state update goes in the same PR.
...
Log to `decisions.md` (global) or the per-product log during the conversation. Announce: "I'm logging this as D-### Type [1/2]." Do not defer to session end.
...
- ❌ Putting tactical day-to-day plans in product specs (commit churn)
