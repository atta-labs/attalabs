...
## D-030 — AEG enhancements: provenance export, spec-conformance review (advisory), observe mode
...
**Ratified by:** Principal (ratified 2026-06-10; spec-conformance amended from BLOCKER to advisory before ratification)
...
2. **Spec-conformance review — advisory, never blocking** (from Augment Intent's Verifier). The code Reviewer additionally cross-checks the diff against the `Product:`-named spec in `apps/*/specs/`, not only the brief — a diff can satisfy its brief and still diverge from the product's specced behavior, and noticing that is useful. **But because specs are mostly unratified and out-of-flow (D-005), a stale or draft spec must never be able to veto correct code** — so spec divergence is surfaced as an **advisory finding** and, where it matters, a `severity:strategy` escalation ("is the spec stale, or the code wrong?"), and is **never a BLOCKER**. Only brief-conformance, correctness, safety, scope, and honest-tests can block a PR. This adds **no new persistent artifact** — it reads the product spec that already exists, as-written. (`roles/reviewer.md`; `state-machine.md` §3, §5.) *(Amended from the originally-drafted BLOCKER severity to advisory before ratification — see "Amendment" below.)*
...
**Amendment (pre-ratification, 2026-06-10):** the spec-conformance check (#2) was originally drafted with a spec **contradiction = BLOCKER**. That was corrected to **advisory (never a blocker)** before ratification: since the spec-ratification mechanism (D-005) has not been run and most specs are `draft`/out-of-flow, allowing a spec to block a merge would let a stale, non-authoritative artifact veto correct code — the inverse of the intended safeguard. The deeper, blocking-capable form of spec verification is deferred to the future **Integrity Reviewer** (iteration-close, deep coherence read against *ratified* specs only), sequenced behind a spec refresh-and-ratify pass — see `docs/ecosystem-backlog.md`.
...
- Spec-conformance as a hard BLOCKER: rejected (see Amendment) — specs are unratified/out-of-flow, so they must not gate merges; divergence is advisory.
...
**Consequences:**
- `roles/reviewer.md` — spec-conformance cross-check added (check #2), **advisory**; `SPEC CONFORMANCE` line in the verdict; spec divergence is a finding + `severity:strategy` escalation, never a BLOCKER; only brief-conformance/correctness/safety/scope/tests block.
...
- `state-machine.md` — §2 (provenance as a Class 2 close-out projection + matrix row), §3 (Reviewer advisory spec-conformance authority), §5 (Reviewer reads the spec as-written, advisory), §12 (observe mode as the advisory floor; provenance assembly as trusted discipline), §13 (provenance block append-only).
- All in PR #83 (`feat/aeg-provenance-verifier-observe`), Tier 3, docs-only.
- Ratified 2026-06-10 (PENDING → ACTIVE). All three additions are **trusted discipline** today (no new CI gate); automation is future work. The deeper "verify against a living spec" form of the verifier is the future Integrity Reviewer, not part of this decision.
- Follow-ups noted as backlog candidates, not decided here (`docs/ecosystem-backlog.md`): (a) a spec refresh-and-ratify pass starting with Vāda; (b) the Integrity Reviewer (iteration-close deep coherence read, depends on (a)); (c) a regulation → AEG-mechanism mapping doc (EU AI Act / SLSA / NIST / SOC 2) turning the provenance output into compliance evidence, requiring primary-source verification before action.
