...
The model layer (constitution, flow, roles, skills, process) exists **once**, at repo-root `project-management/`. A unit's `project-management/` carries only that unit's *living state* — never a copy of the model. The AEG skills are part of the model and live at `project-management/skills/` (canonical); the `.claude/skills/` copies are a generated view (D-039).
