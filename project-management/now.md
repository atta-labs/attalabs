...
- Per-product: `apps/vada-ai/specs/vada-backlog.md` · `apps/cetana-ai/specs/cetana-backlog.md` · `apps/herald-ai/specs/herald-backlog.md`
- Cross-cutting / AEG-itself: `docs/ecosystem-backlog.md`
- **Active execution:** `project-management/iterations/` — a slice *pulled from a backlog* when it's actually being built. (None active right now.)
...
1. **AEG UI** — the deployed product that visualizes an iteration's tasks (status derived from the forge). This is the intended **first real iteration** — decompose into its own task graph (derive module, GitHub App auth, webhook fact-cache, attention queue, repo-rollup, DAG view).
